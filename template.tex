...
% débordent dans les colonnes de tableau et en marge.
% Stratégie en trois temps :
%   (a) on donne à LaTeX une marge d'élasticité d'urgence + \sloppy,
%   (b) on réactive la coupure aux tirets dans la fonte monospace,
%   (c) on enveloppe \texttt avec \seqsplit, qui autorise une coupure
%       après chaque caractère du contenu (sans changer le rendu si
%       le mot tient sur la ligne).
...
% Redéfinit \texttt : conserve le rendu mono et autorise les coupures
% à n'importe quelle position si nécessaire.
\let\PandocOrigTexttt\texttt
\renewcommand{\texttt}[1]{\PandocOrigTexttt{\seqsplit{#1}}}
\makeatother
