...
Un point de vocabulaire est à clarifier d'emblée : trois « configurations de référence » distinctes coexistent dans la suite, parce qu'elles servent trois finalités différentes. La première est la *configuration du POC déployé* (`markdown-1200-50` + `ada-002` + dense + top-$k$ = 10 avec seuil de similarité 0,83), décrite au Ch. 7.6 : c'est celle qui tourne effectivement en production interne. La deuxième est la *configuration de référence pour la stabilité* (`markdown-1200-50` + `ada-002` + `dense-k5-thresh` + `azure-gpt35`), utilisée au § 8.4 : elle reprend la chaîne du POC, mais réduit le top-$k$ à 5 pour rester aligné sur la variante effectivement *benchmarkée* parmi les six du § 8.1.1. La troisième est la *meilleure configuration identifiée dans la campagne de génération* (`recursive-512-64` + `ada-002` + `hybrid-k5` + `azure-gpt35`), utilisée à partir du § 9.1 comme cible pour l'analyse d'erreurs : elle correspond à la chaîne candidate pour la prochaine itération du POC, pas à celle actuellement déployée. Ce choix est délibéré : analyser les erreurs sur la configuration en route vers la production est plus actionnable qu'analyser celles d'une configuration appelée à évoluer.
