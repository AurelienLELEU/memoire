...
- la **production des indicateurs** et le pilotage de la santé-sécurité (taux de fréquence, taux de gravité, nombre d'heures travaillées, nombre d'accidents à haut potentiel (HiPo)) ;
...
- L'hétérogénéité documentaire rend les *benchmarks* publics insuffisants. ScribBERT ne peut pas être évalué avec les jeux de données académiques classiques. Il a fallu construire un corpus de test interne.
...
L'étape de *retrieval* peut également être suivie d'un *reranking* : un ensemble large de candidats est d'abord récupéré (rapide), puis un modèle plus précis (souvent un *cross-encoder*) reclasse finement les passages. (J'y reviendrai au Chapitre 4)
...
Pour chiffrer chaque catégorie d'erreur sur les 50 questions, j'utilise les scores RAGAS calculés question par question (*context recall*, *context precision*, *faithfulness*) comme indicateurs : un seuil simple sur ces scores permet de classer chaque question dans la bonne catégorie, et un script compte ensuite combien de questions tombent dans chacune.
