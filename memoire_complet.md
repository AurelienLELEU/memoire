...
Mes remerciements vont également à Nicolas Derrez et Hervé Dit Lebas pour le prêt de la machine de test (Dell GB10) qui a rendu possibles les expérimentations locales sur modèles auto-hébergés, et plus largement pour la confiance qu'ils m'ont accordée en mettant ce matériel à disposition.
...
Seulement voilà : dans le domaine de la santé-sécurité, un effet "ouahou" ne suffit pas. La qualité de l'information transmise engage directement la sécurité des compagnons sur les chantiers. Une réponse plausible mais fausse, une procédure mal citée, une obligation transformée en simple recommandation. Les conséquences potentielles dépassent l'enjeu technique. Chaque réponse doit être exacte, fondée sur les bonnes sources, et vérifiable. Cette exigence conduit immédiatement à la question centrale de ce mémoire : **comment évaluer la cohérence et la fiabilité d'un système RAG ?**
...
Enfin, une autre famille importante, très utilisée en pratique, est celle des modèles de langage pour l'IR, dans laquelle la probabilité qu'un document génère une requête est estimée (approches *query likelihood*), avec des techniques de lissage et de feedback pseudo-pertinent.[@PonteCroft1998; @LavrenkoCroft2001]
...
L'étape de *retrieval* peut également être suivie d'un *reranking* : un ensemble large de candidats est d'abord récupéré (rapide), puis un modèle plus précis (souvent un *cross-encoder*) reclasse finement les passages. (Ce point est repris au Chapitre 4.)
...
Dans le cas d'un RAG, l'évaluation pertinente doit idéalement être décomposable : elle doit permettre de dire *où* se situe l'échec (*retrieval*, *reranking*, *prompt*, génération) et pas seulement constater que la réponse finale est "bonne" ou "mauvaise".

Pour expliciter la suite, quelques définitions courantes du côté *retrieval* sont rappelées, sur un ensemble de requêtes $Q$. Soit $\mathrm{TopK}(q)$ l'ensemble des $k$ premiers passages récupérés pour la requête $q$, et $\mathrm{Rel}(q)$ l'ensemble des passages pertinents (selon l'annotation).
...
- séparer les erreurs de *retrieval* et les erreurs de génération, parce que leurs corrections obéissent à des logiques différentes,
...
Le paysage des modèles d'*embedding* évolue rapidement. Au moment de l'écriture, plusieurs familles peuvent être distinguées :
...
Les embeddings "Matryoshka" (*Matryoshka Representation Learning*) permettent de tronquer la dimension à posteriori avec une perte limitée, offrant un curseur qualité/coût ajustable sans réindexation complète.
...
Deux niveaux d'évaluation se distinguent pour un modèle d'*embedding* :
...
| Coût | API payante (OpenAI, Cohere) ou auto-hébergé (GPU (*Graphical Processing Unit*)) ? |
...
- Normalisation : unification des guillemets, des espaces insécables, des tirets ; éventuellement passage en minuscules pour le *sparse retrieval* (mais pas pour les *embeddings*, qui sont généralement sensibles à la casse).
...
1. *Retrieval* initial pour obtenir le top-100 des candidats (rapide, $O(\log N)$ sur HNSW),
...
C'est un point qui a nécessité plusieurs itérations concrètes. La première version de *prompt* système était beaucoup trop permissive : sur des questions qui n'avaient rien à voir avec la santé-sécurité, le modèle sortait des recettes de cookies, répondait à partir de sa connaissance générale, et oubliait de citer ses sources. Les instructions ont dû être durcies progressivement : cadrer explicitement le domaine, exiger une citation, autoriser et même encourager le "je ne sais pas" lorsque le contexte ne contient pas l'information. C'est en partie ce qui a permis de comprendre que le *prompt* fait autant partie de l'évaluation que le modèle ou l'*embedding*.
...
C'est le premier maillon de la chaîne, et si le *retrieval* rate la bonne règle, rien dans la suite ne peut compenser. Plusieurs métriques permettent de le mesurer, selon l'angle visé :
...
- Vectorielles (BERTScore, BLEURT, similarité cosinus des *embeddings* de réponse) : capturent mieux la similarité sémantique. Limitation : peuvent juger "proches" deux réponses dont l'une contient une erreur factuelle subtile.[@Zhang2020BERTScore; @Sellam2020BLEURT]
...
Ces sources de variance n'ont pas pu être testées systématiquement sur ScribBERT dans le cadre de ce travail (cf. limites, Ch. 10). La cartographie ci-dessous reste donc en partie théorique, fondée sur la littérature et sur quelques observations ponctuelles pendant le développement.
...
| Calcul tensoriel | PyTorch (via `sentence-transformers` et `transformers`) | Standard de fait pour l'inférence des modèles d'*embedding* et des LLMs *open-source* ; bonne intégration GPU sur le *cluster* |
...
| *Frontend* | ReactJS (codé à la main) | Contrôle total de l'expérience utilisateur, intégration avec la charte graphique interne, pas de dépendance à un *framework* sans-code |
...
- la cible de taille est d'environ 1 200 tokens par *chunk* (variante `markdown-1200-50` du *benchmark*, retenue en POC), ce qui correspond empiriquement à un compromis entre :
  ...
  - le *benchmark* systématique (§ 8a.2) montre que les chunkings 1024 *tokens* dominent en MRR, ce qui valide à posteriori l'ordre de grandeur retenu ;
...
| *Chunking* | *Embedding* | *Retrieval* | MRR | Hit@5 | Recall@5 | nDCG@5 |
|----------|-----------|-----------|-----|-------|----------|--------|
...
La campagne génération a porté sur cinq configurations choisies comme représentatives, croisant trois *pipelines* Azure et deux *pipelines* local-Mistral-7B, toutes évaluées par RAGAS. Chaque *pipeline* a généré une réponse aux 50 questions du jeu de test. Synthèse :
...
Quatre lectures se dégagent. D'abord, la configuration `hybrid-k5` domine sur les quatre scores RAGAS : elle obtient simultanément la meilleure *faithfulness* (0,765), la meilleure *answer relevancy* (0,756), la meilleure *context precision* (0,687) et le meilleur *context recall* (0,629), tout en étant la plus rapide côté génération (5,09 s médian). C'est la confirmation, côté génération cette fois, du gain d'hybridation déjà observé côté *retrieval* (§ 8a.2) : un *retrieval* plus précis se traduit par une génération à la fois plus fidèle aux sources et mieux ciblée sur la question.

Deuxième lecture, plus contre-intuitive : la variante `dense-k5-neigh` améliore la fidélité par rapport à `dense-k5-thresh` (+0,024 de *faithfulness*, +0,020 d'*answer relevancy*) malgré sa dégradation observée en *retrieval* pur (*benchmark* precedents, § 8a.2). Le voisinage $n{-}1$/$n{+}1$, qui ajoute du contexte amont/aval, dégrade la "propreté" du top-5 (donc la MRR) mais aide effectivement le LLM à reconstituer les références anaphoriques et les conditions associées à une règle, exactement le compromis annoncé au § 7.4. C'est une illustration concrète du découplage *retrieval* pur ≠ utilité pour la génération discuté au Ch. 5.5.
...
Enfin, la configuration `dense-k5-thresh` (seuil de distance maximal fixé à 0,17) joue effectivement son rôle de garde-fou : environ 5 questions sur 50 se retrouvent avec un contexte vide ou très partiel et reçoivent une réponse de type "information non trouvée dans les référentiels". Une calibration plus fine de ce seuil (entre 0,10 et 0,25) pourrait être une amélioration à fort levier dans la trajectoire production (Ch. 10).

### 8a.6. Coût opérationnel

Les latences mesurées sur l'ensemble du *benchmark* se décomposent comme suit. La colonne `latency_s` du *benchmark* *retrieval* mesure uniquement l'appel d'*embedding* de la requête (et l'aller-retour web pour les modèles via API Azure), sans inclure la recherche ChromaDB elle-même (négligeable sur un index de cette taille). Les temps de génération discutés au § 8a.3 (colonnes `t_retrieval_s` et `t_generation_s` du *benchmark* génération) sont mesurés séparément et reportés ici dans le tableau du bas pour donner une vision *end-to-end*.

Côté *retrieval* (médianes par modèle d'*embedding* sur les configurations `dense-k5`) :
...
- Construction des index vectoriels : seuls les modèles de vectorisation propriétaires (`ada-002`, `embed-3-large`) ont engendré un coût API. Avec 12 variantes de *chunking* x 2 modèles de vectorisation sur un corpus d'environ 2 millions de *tokens*, cela représente environ 48 millions de *tokens* embeddés cumulés : environ 2,5 € pour `ada-002` (0,0001 €/1k *tokens*) et environ 3 € pour `embed-3-large` (0,00013 €/1k *tokens*). Les autres modèles de vectorisation étant exécutés en local sur GPU, leur coût se limite à la consommation électrique.
- *Embeddings* de requête sur la campagne *retrieval* : 750 configurations x 50 questions = 37 500 requêtes, dont environ 250 configurations utilisaient un modèle de vectorisation API, cela représente moins de 1 € cumulé.
...
- Total estimé : environ 15 à 20 € pour l'ensemble de la phase exploratoire Azure, hors coût matériel/électrique du GPU local utilisé pour les modèles de vectorisation *open-source* et pour les deux runs Mistral-7B.
...
La phase de test utilisateur menée pendant le projet a recueilli des retours majoritairement positifs. Ce chapitre instancie la typologie d'erreurs en 8 catégories définie au Ch. 5.5.4 (*retrieval* miss, *retrieval* bruit, hallucination factuelle, omission d'exception, inversion de modalité, contradiction silencieuse, refus à tort, hors-périmètre accepté) sur les sorties de la configuration de référence (`recursive-512-64` + `ada-002` + `hybrid-k5` + `gpt-3.5-turbo`) sur les 50 questions du jeu de test
...
- *Retrieval* miss : *context recall* inférieur à 0,30, signe que les *chunks* attendus ne sont pas dans le top-5.
- *Retrieval* bruit : *context precision* inférieur à 0,30 alors que le top-5 contient bien des *chunks*, signe d'un top-5 dilué.
...
|--------------------|------------------------|-----------------------|--------------------------------------------------|
| *Retrieval* miss | 10/50 (20 %) | Q003 « Que faire si l'O2 d'un espace confiné est < 19,5 % ? » le *chunk* de `REF-2223` citant le seuil sort du top-5, remplacé par des *chunks* plus généraux sur la ventilation | Le *chunk* normatif (court, dense) est mal classé face aux *chunks* longs qui répètent le concept large « espace confiné ». Ajouter à l'index une paraphrase titre + résumé pour chaque référentiel, et tester `dense-k20-rerank5`|
| *Retrieval* bruit | 9/50 (18 %) | Q005 « Différence entre permis de feu et permis d'intervention ATEX ? » top-5 dominé par le « permis de feu », un seul *chunk* ATEX | Sur les questions comparatives, l'entité la plus représentée dans le corpus noie la seconde. Décomposer en deux sous-requêtes (Ch. 4.3.6) puis fusionner les *top-k*, ou appliquer un *cross-encoder* à chaque moitié du contexte |
...
| Contradiction silencieuse | 0/50 |  | Aucune question de la config de référence ne présente la combinaison faith = 0 + réponse longue non-refus. Q046, faith 0, déclenche en pratique un refus standardisé plutôt qu'une réponse fabriquée (à surveiller toutefois sur d'autres docs courts EN). Maintenir le garde-fou applicatif à instrumenter en production : « réponse longue + score moyen de pertinence faible = warning UI ou refus » |
...
Lecture transverse : sur les 50 questions, 13 sont concernées par au moins une erreur de *retrieval* (miss ou bruit, certaines cumulant les deux), soit 26 %. C'est cohérent avec un Hit@5 de 0,80 sur la config (§ 8a.3) et confirme que le levier principal d'amélioration reste le *retrieval* plutôt que la génération. Sur le sous-ensemble où le *retrieval* est correct, la *faithfulness* moyenne grimpe à 0,88, comparé à 0,77 sur l'ensemble.
...
Les systèmes RAG s'installent rapidement dans les usages internes des entreprises, mais leur évaluation reste un chantier largement ouvert. Ce mémoire a cherché à y apporter une contribution modeste mais opérationnelle : un cadre d'évaluation transférable, qui considère la fiabilité non comme une étiquette à apposer, mais comme une propriété à mesurer dimension par dimension, à éprouver dans le temps et à gouverner au même titre que n'importe quel autre indicateur de performance.

Au moment de la dernière révision de ce mémoire, le projet ScribBERT vient d'être validé pour un passage en industrialisation à l'échelle de Bouygues Construction. C'est une forme de validation concrète du travail mené pendant ces deux années, et surtout l'occasion de mettre à l'épreuve, sur un périmètre élargi (plus de filiales, plus de langues, plus d'utilisateurs), le protocole décrit ici. Les questions qui restent ouvertes sont nombreuses : comment faire vivre un jeu de test annoté quand le corpus évolue en continu ? Comment évaluer la stabilité dans la durée plutôt qu'au moment du déploiement ? Comment articuler l'évaluation automatisée à un retour utilisateur structuré sans en faire un goulot d'étranglement ? Ces questions dépassent le cadre de ce mémoire, mais elles en constituent le prolongement naturel et représentent, à notre sens, l'agenda de recherche le plus utile pour les RAG d'entreprise dans les prochaines années.
