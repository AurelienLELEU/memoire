...
Le travail s'articule en trois parties. La première replace le RAG dans la lignée historique de la recherche d'information et formalise les notions de pertinence, de cohérence et de fidélité aux sources. La deuxième construit un protocole d'évaluation reproductible structuré autour de cinq dimensions de la fiabilité (pertinence du [*retrieval*](#gloss-retrieval), fidélité aux sources, pertinence de la réponse, stabilité, traçabilité), combinant métriques automatiques (Recall@k, MRR, nDCG, [*faithfulness*](#gloss-faithfulness) via [*LLM-as-judge*](#gloss-llm-as-judge)) et validation humaine ciblée. La troisième instancie ce protocole sur ScribBERT : [*benchmark*](#gloss-benchmark) des stratégies de [*chunking*](#gloss-chunking), des modèles de vectorisation et des configurations de [*retrieval*](#gloss-retrieval), analyse d'erreurs typologique et étude de stabilité inter-runs.
...
**Mots-clés :** RAG (*Retrieval-Augmented Generation*), évaluation de systèmes IA, fiabilité, [*chunking*](#gloss-chunking), vectorisation, [*LLM-as-judge*](#gloss-llm-as-judge), santé-sécurité, [*retrieval*](#gloss-retrieval), stabilité, protocole d'évaluation
...
- [Partie I](#partie-1) - Cadre conceptuel et théorique : les fondements du RAG, l'histoire de la recherche d'information, et les notions de pertinence et de cohérence qui sous-tendent l'évaluation.
- [Partie II](#partie-2) - Méthodologie d'évaluation : construction du protocole, choix des métriques, conditions expérimentales.
- [Partie III](#partie-3) - Application et discussion : mise en œuvre sur ScribBERT, résultats, et recommandations.
...
- l'hétérogénéité documentaire rend les [*benchmarks*](#gloss-benchmark) publics inopérants, et impose la construction d'un corpus de test interne ;
...
## De la recherche documentaire à la recherche sémantique {#ch1}

### Brève histoire de la recherche d'information : du lexical au probabiliste {#sec1-1}
...
Les modèles vectoriels ont ensuite introduit une représentation plus graduelle : documents et requêtes sont représentés comme des vecteurs, et la similarité est souvent mesurée via un calcul de similarité vectorielle par cosinus. Une pondération bien connue est le TF-IDF, qui combine une mesure de fréquence locale (*term frequency*) et une mesure de rareté globale (*inverse document frequency*). Formellement :
...
L'idée d'IDF comme signal de discrimination d'un terme remonte à des travaux fondateurs sur le [*term specificity*](#gloss-term-specificity)[@SparckJones1972]. Le vector space model (VSM) popularisé par Salton et al. a ensuite fourni un cadre pratique et encore omniprésent pour pondérer et comparer requêtes et documents [@Salton1975].
...
Enfin, une autre famille importante, très utilisée en pratique, est celle des modèles de langage pour l'IR, dans laquelle la probabilité qu'un document génère une requête est estimée (approches [*query likelihood*](#gloss-query-likelihood)), avec des techniques de lissage et de retour pseudo-pertinent[@PonteCroft1998; @LavrenkoCroft2001].

Ces modèles "classiques" (BM25, [*query likelihood*](#gloss-query-likelihood), variantes) restent extrêmement compétitifs, notamment sur des corpus techniques où les indices lexicaux (références, numéros de procédure, intitulés normatifs) apportent des signaux précieux.

#### Évaluer un système de recherche : pourquoi les métriques comptent {#sec1-1-1}
...
#### Retour, reformulation et *learning-to-rank* {#sec1-1-2}

Les systèmes IR peuvent aussi reformuler la requête pour améliorer les résultats. Le [*relevance feedback*](#gloss-relevance-feedback) et ses variantes (pseudo-relevance feedback, expansion de requête) augmentent le rappel (*recall*) mais peuvent introduire du bruit [@Rocchio1971]. Dans un RAG, ce compromis est amplifié : une expansion mal calibrée risque de récupérer des passages thématiquement proches mais non applicables.

En parallèle, le [*learning-to-rank*](#gloss-learning-to-rank) a permis d'apprendre des fonctions de classement à partir de données (clics, jugements), avec des approches [*pointwise*](#gloss-pointwise), [*pairwise*](#gloss-pairwise) et [*listwise*](#gloss-listwise) [@Liu2009LTR]. Les systèmes industriels combinent aujourd'hui une récupération rapide, un [*reranking*](#gloss-reranking) plus coûteux (souvent [*cross-encoder*](#gloss-cross-encoder)) et des signaux métier (popularité, fraîcheur). Le RAG s'insère dans cette logique multi-étage : la majorité des architectures dites [*Advanced RAG*](#gloss-advanced-rag) documentées dans la littérature reprennent ce schéma à plusieurs niveaux (récupération initiale large, filtrage, [*reranking*](#gloss-reranking), génération conditionnée), parfois enrichi de boucles de réflexion ou de mécanismes de décision sur la nécessité de récupérer [@Gao2024RAGSurvey; @NogueiraCho2019].

```{=latex}
\newpage
```

### Limites du matching lexical {#sec1-2}
...
### Vers la recherche sémantique : représentations distribuées et vectorisations {#sec1-3}
...
Les modèles de type Transformers, et BERT en particulier, ont "résolu" ce problème en introduisant des représentations contextualisées : la représentation d'un [*token*](#gloss-token) dépend désormais de la phrase entière [@Vaswani2017; @Devlin2019]. C'est ce qui a ouvert la voie à la recherche sémantique moderne.

Dans la pratique, l'usage IR/RAG requiert surtout des vectorisations de phrases/passages (*sentence or passage embeddings*). Les approches de type bi-encodeur (ou [*dual-encoder*](#gloss-dual-encoder)) encodent requête et passage séparément, puis comparent leurs vecteurs (souvent cosinus ou produit scalaire). Sentence-BERT (SBERT) a été une contribution clé pour obtenir des vectorisations de phrases efficaces via apprentissage contrastif et [*siamese networks*](#gloss-siamese-networks) [@ReimersGurevych2019]. Des travaux plus récents (ex. SimCSE) montrent que des schémas contrastifs simples peuvent déjà produire de très bons espaces de vectorisation [@Gao2021].

À l'inverse, les [*cross-encoders*](#gloss-cross-encoder) concatènent requête et passage et produisent un score de pertinence en tenant compte finement des interactions token-à-token, mais ils coûtent beaucoup plus cher à l'inférence. Ils sont souvent utilisés en [*reranking*](#gloss-reranking) sur un petit nombre de candidats [@NogueiraCho2019].

Enfin, des architectures intermédiaires ([*late interaction*](#gloss-late-interaction)) comme ColBERT cherchent à concilier précision (interactions fines) et efficacité (indexation) via des représentations token-level compressées [@KhattabZaharia2020].

### Sparse, dense et hybride : familles de récupération {#sec1-4}
...
L'étape de récupération peut également être suivie d'un [*reranking*](#gloss-reranking) : un ensemble large de candidats est d'abord récupéré (rapide), puis un modèle plus précis (souvent un [*cross-encoder*](#gloss-cross-encoder)) reclasse finement les passages. (Ce point est repris au [Chapitre 4](#ch4).)

Au-delà de cette typologie, un point technique essentiel pour les systèmes denses est l'indexation par recherche du plus proche voisin approximatif ([*Approximate Nearest Neighbor*](#gloss-approximate-nearest-neighbor), ANN). À grande échelle, il est impossible de comparer une requête à tous les vecteurs. Des structures dédiées (HNSW, IVF, PQ…) sont donc utilisées pour accélérer la recherche au prix d'une approximation contrôlée [@MalkovYashunin2018; @Johnson2019].
...
- erreur de formulation de requête ([*query rewriting*](#gloss-query-rewriting) absent ou mal calibré).

### Problématiques spécifiques à la sémantique en contexte technique {#sec1-5}
...
### Neural IR et récupération dense {#sec1-7}
...
La question pratique pour un cas comme ScribBERT est directe : **un modèle entraîné sur des données web généralistes est-il adapté à un vocabulaire métier ?** Le [*benchmark*](#gloss-benchmark) BEIR a montré une dégradation significative des performances hors domaine d'entraînement [@Thakur2021BEIR]. Cette question sera traitée en [Partie II](#partie-2).

### De la récupération dense au RAG : la convergence historique {#sec1-8}

La trajectoire décrite dans ce chapitre, du lexical aux vectorisations puis à la récupération dense, mène à l'idée de coupler un [*retriever*](#gloss-retriever) dense à un modèle génératif. Sur le moment, chaque étape a nécessité des contributions distinctes, et ce n'est qu'à partir de 2020 que la filiation devient explicite.

Le paradigme [*retriever-reader*](#gloss-retriever-reader) a d'abord été popularisé par DrQA (Chen et al., 2017), puis ORQA [@Lee2019ORQA] et REALM [@Guu2020] ont progressivement intégré le [*retriever*](#gloss-retriever) dans la boucle d'apprentissage. Le RAG (Lewis et al., 2020) couplait un générateur BART à un [*retriever*](#gloss-retriever) DPR avec deux variantes : RAG-Sequence, RAG-Token [@Lewis2020], et Fusion-in-Decoder (Izacard & Grave, 2021) a ensuite montré que l'injection de davantage de passages dans le décodeur permettait d'améliorer encore les résultats [@IzacardGrave2021].
...
## Les fondements du RAG (*Retrieval-Augmented* Generation) {#ch2}

### Principe général : génération augmentée par récupération {#sec2-1}
...
### RAG vs *fine-tuning* : choix méthodologiques {#sec2-2}

La question "pourquoi un RAG plutôt qu'un [*fine-tuning*](#gloss-fine-tuning) ?" est revenue plusieurs fois dans les discussions autour de ScribBERT, et le choix s'est fait assez naturellement, mais il mérite d'être explicité.

Le [*fine-tuning*](#gloss-fine-tuning) consiste à adapter les poids d'un modèle sur des données spécifiques. Le modèle obtenu "sait" les choses directement, sans avoir besoin de consulter des documents au moment de la requête. En théorie cela semble convaincant. En pratique, dans un contexte comme le nôtre, c'est difficilement tenable : nos procédures évoluent régulièrement, et réentraîner un modèle à chaque mise à jour documentaire serait trop coûteux et non traçable (les sources mobilisées par le modèle pour répondre resteraient inconnues). Une étude comparative récente confirme ce constat empiriquement : sur des tâches d'injection de connaissances nouvelles, le RAG surpasse systématiquement le [*fine-tuning*](#gloss-fine-tuning) supervisé, et plus encore lorsque l'information est rare ou évolutive [@Ovadia2024FTvsRAG].
...
Cela dit, le RAG ne nous interdit pas de faire du [*fine-tuning*](#gloss-fine-tuning). Un [*fine-tuning*](#gloss-fine-tuning) léger peut servir à calibrer le ton, tandis que le RAG gère l'accès aux connaissances. La littérature récente insiste d'ailleurs sur cette complémentarité [@Gao2024RAGSurvey].

### Architecture type d'une chaîne de traitement RAG {#sec2-3}
...
2. [*Chunking*](#gloss-chunking) : découpage en segments ([*chunks*](#gloss-chunk)).
3. Vectorisation / indexation : calcul de vectorisations pour chaque [*chunk*](#gloss-chunk) et insertion dans un index (base vectorielle).
4. Récupération / [*reranking*](#gloss-reranking) : récupération de $k$ passages pertinents (qui peuvent être dans un second temps reclassés par un modèle plus fin ([*reranking*](#gloss-reranking))).
5. Génération : construction d'un [*prompt*](#gloss-prompt) complet avec la requête + contexte, puis génération d'une réponse.

Pour ScribBERT, cette architecture se décline avec des choix d'implémentation qui seront décrits en [PARTIE III](#partie-3).

Un mot sur le [*chunking*](#gloss-chunking), qui est souvent décrit comme un paramètre "d'ingestion" mais correspond en réalité à un choix de modélisation : quelle est l'unité minimale (et maximale également) de connaissance que le système peut retrouver et citer ?
...
Le [*chunking*](#gloss-chunking) influence directement :

- le rappel ([*chunks*](#gloss-chunk) trop gros : moins d'unités, risque de dilution ; [*chunks*](#gloss-chunk) trop petits : manque de contexte),
...
Ces aspects seront étudiés dans la [PARTIE III](#partie-3) (comparaisons de [*chunking*](#gloss-chunking)).

### Les avantages du RAG {#sec2-4}
...
Les autres avantages sont plus opérationnels : les connaissances peuvent être mises à jour sans réentraîner le modèle (les documents modifiés sont simplement réindexés), le corpus interne reste privé (pas besoin de l'envoyer dans un service cloud pour du [*fine-tuning*](#gloss-fine-tuning)), et le coût global est moindre qu'un entraînement dédié.

### Les défis du RAG : bruit, contradictions et cohérence {#sec2-5}

Pour autant, le RAG n'est pas une solution magique. Il introduit ses propres difficultés :

Le bruit documentaire est probablement le problème le plus fréquent : la récupération ramène des [*chunks*](#gloss-chunk) qui sont sémantiquement proches de la requête mais qui ne sont pas applicables au cas précis. Dans notre corpus, c'est relativement fréquent avec des procédures qui partagent une terminologie commune mais s'appliquent à des situations différentes.
...
La dépendance au [*chunking*](#gloss-chunking) est un problème plus subtil mais réel : une mauvaise segmentation peut couper une règle en deux, ou séparer une condition de son exception, et le modèle génère alors une réponse incomplète sans diagnostic facile de la cause.
...
En pratique, les systèmes robustes adoptent une architecture [*multi-stage*](#gloss-multi-stage) qui répond directement à ce dilemme : une récupération large (top-$k$ élevé) maximise le rappel, un [*reranking*](#gloss-reranking) par [*cross-encoder*](#gloss-cross-encoder) augmente ensuite la précision sur ce petit ensemble candidat[@NogueiraCho2019], et une sélection/assemblage finale respecte la limite de contexte du générateur. Chaque étage a un effet direct sur la fidélité : une récupération trop large sans [*reranking*](#gloss-reranking) augmente le bruit, un [*reranking*](#gloss-reranking) mal calibré peut favoriser des passages "proches" mais moins normatifs, et une sélection trop agressive peut écarter des passages qui auraient été utiles.

### Ancrage, citations et attribution : de la preuve à la confiance {#sec2-6}

Citer une source ne suffit pas. Les tests menés sur ScribBERT ont révélé des cas où le système citait un document qui n'avait qu'un rapport lointain avec la question, ce qui est pire que l'absence de citation, car cela donne une illusion de rigueur. La littérature formalise cette intuition en distinguant trois dimensions : la [*context relevance*](#gloss-context-relevance) (le contexte récupéré est-il utile ?), l'[*answer relevance*](#gloss-answer-relevance) (la réponse traite-t-elle la question ?) et la [*faithfulness*](#gloss-faithfulness) (la réponse est-elle supportée par le contexte ?). Ces trois dimensions ne se recouvrent pas, et c'est précisément ce qui rend l'évaluation complexe.

### RAG et mémoire : connaissances paramétriques vs non-paramétriques {#sec2-7}
...
### Définir la pertinence : une notion multi-dimensionnelle {#sec3-1}
...
### Définir la cohérence : du texte à la fidélité aux sources {#sec3-2}
...
Il est utile de distinguer trois notions proches mais différentes. La cohérence textuelle caractérise un texte qui "se tient" linguistiquement, avec une dimension locale (connecteurs, anaphores, absence de contradictions phrase-à-phrase) et une dimension globale (fil directeur sur l'ensemble de la réponse) ; les LLMs modernes maîtrisent bien la première, beaucoup moins la seconde quand le contexte est hétérogène ou que le [*prompt*](#gloss-prompt) impose un format strict. La factualité dit que les propositions sont vraies dans un cadre documentaire de référence. La fidélité (ou [*groundedness*](#gloss-groundedness)) est plus restrictive : les propositions doivent être justifiées par les sources effectivement fournies au modèle. Dans un RAG, cette dernière est souvent plus importante que la factualité absolue : le système ne doit pas dépasser ce que le corpus permet d'affirmer.
...
### Définir la fiabilité : une synthèse opératoire {#sec3-3}
...
1. Elle décompose la fiabilité en dimensions mesurables, ce qui permet d'organiser le protocole d'évaluation ([Chapitre 5](#ch5)) autour de chacune.
...
### Pertinence perçue vs pertinence mesurée {#sec3-4}
...
### Travaux récents sur l'évaluation des RAG et LLMs augmentés {#sec3-5}
...
3. Évaluation "[*end-to-end*](#gloss-end-to-end)" : [*frameworks*](#gloss-framework) dédiés au RAG (ex. RAGAS, TruLens, LangSmith) qui tentent de décomposer la qualité en sous-scores ([*context relevance*](#gloss-context-relevance), [*answer relevance*](#gloss-answer-relevance), [*faithfulness*](#gloss-faithfulness), citation, etc.).
4. [*LLM-as-judge*](#gloss-llm-as-judge) : utiliser un LLM pour noter des réponses selon une grille (G-Eval, Prometheus). Puissant mais nécessite une gouvernance stricte (biais, fuite d'informations, reproductibilité).

Les [*benchmarks*](#gloss-benchmark) de récupération généralistes (BEIR) et les [*leaderboards*](#gloss-leaderboards) de vectorisations (MTEB) ont également contribué à standardiser la comparaison de modèles et à clarifier l'écart entre performance sur des tâches "web" et performance sur des corpus spécialisés [@Thakur2021BEIR; @Muennighoff2023MTEB].
...
Dans le cas d'un RAG, l'évaluation pertinente doit idéalement être décomposable : elle doit permettre de dire *où* se situe l'échec (récupération, [*reranking*](#gloss-reranking), [*prompt*](#gloss-prompt), génération) et pas seulement de constater que la réponse finale est "bonne" ou "mauvaise".
...
- MRR ([*Mean Reciprocal Rank*](#gloss-mean-reciprocal-rank)), utile lorsqu'au moins un bon passage est attendu parmi les premiers résultats :
...
### Positionnement de la contribution du mémoire {#sec3-6}
...
- intégrer les spécificités santé-sécurité (criticité, exceptions) que les [*benchmarks*](#gloss-benchmark) généralistes ignorent,
- rester reproductible et applicable sur un corpus d'entreprise,
- produire des diagnostics actionnables : pas seulement "c'est bon" ou "c'est pas bon", mais *quoi* améliorer (le [*chunking*](#gloss-chunking), le [*top-k*](#gloss-top-k), le [*reranking*](#gloss-reranking), le [*prompt*](#gloss-prompt), la température).

La [Partie II](#partie-2) présente la méthodologie retenue, et la [Partie III](#partie-3) l'applique à ScribBERT.
...
La [Partie I](#partie-1) a posé les bases : ce qu'est un RAG, ce que signifient "pertinence" et "cohérence" dans ce contexte, et pourquoi ces notions sont si délicates à évaluer quand l'enjeu est la sécurité des collaborateurs. La [Partie II](#partie-2) entre dans le concret.

La question de l'évaluation a d'abord paru plus simple qu'elle ne l'est en réalité. Au début du développement de ScribBERT, la démarche relevait du tâtonnement : une configuration était testée, quelques questions étaient posées, et le constat se limitait à savoir si les réponses "avaient l'air bonnes". Sauf que cette approche montre vite ses limites : à chaque modification de paramètre (stratégie de [*chunking*](#gloss-chunking), modèle de vectorisation, valeur de $k$), une question qui marchait bien se dégradait, et une autre qui échouait s'améliorait. Il n'y avait pas de progression nette, pas de signal clair. C'est cette frustration qui a motivé la formalisation d'un protocole d'évaluation rigoureux.

Un fil rouge traverse l'ensemble de cette partie : l'évaluation doit être *décomposable*, c'est-à-dire capable de distinguer ce qui rate dans la couche RAG ([*chunking*](#gloss-chunking), vectorisation, récupération, [*reranking*](#gloss-reranking)) de ce qui rate dans le LLM générateur. C'est cette exigence qui justifie à la fois la décomposition en dimensions ([Ch. 5](#ch5)), le gel des autres composants lors d'une comparaison ([Ch. 5.4](#sec5-4)), et le croisement de plusieurs LLMs sur la même chaîne de récupération (mis en pratique au [§ 8.3](#sec8-3) avec Mistral-7B local vs `gpt-3.5-turbo` Azure). Sans cette discipline, un constat de mauvaise réponse reste imputable à n'importe quel maillon, et aucune correction ciblée n'est possible.

Trois questions structurent cette partie :

1. Quels leviers techniques influencent la qualité d'un RAG, et comment les caractériser ([Chapitre 4](#ch4)) ?
2. Quel protocole mettre en place pour mesurer cette qualité de façon reproductible ([Chapitre 5](#ch5)) ?
3. Comment évaluer la cohérence (fidélité aux sources, stabilité), qui est la dimension la plus difficile à automatiser ([Chapitre 6](#ch6)) ?

L'ambition est de proposer un cadre transférable, pas spécifique à ScribBERT, mais qui sera instancié sur ce cas en [Partie III](#partie-3).

## Modèles et paramètres influençant la performance {#ch4}
...
2. le [*chunking*](#gloss-chunking) et le prétraitement, qui fixent la granularité des unités indexées,
3. les stratégies de récupération (modes de récupération et de classement des passages candidats),
4. la composante de génération (le LLM, le [*prompt*](#gloss-prompt) et les paramètres associés).

### Les modèles de vectorisation {#sec4-1}
...
#### Typologie des modèles disponibles {#sec4-1-1}
...
- Modèles [*open-source*](#gloss-open-source) dérivés de BERT et SBERT : famille `sentence-transformers` (`all-MiniLM-L6-v2`, `all-mpnet-base-v2`, etc.), qui constitue une référence [*open-source*](#gloss-open-source) largement utilisée [@ReimersGurevych2019].
- Modèles multilingues [*open-source*](#gloss-open-source) : `multilingual-e5` (Microsoft), `BGE-M3` (BAAI), `Jina embeddings v3`, qui visent à couvrir un grand nombre de langues avec un seul modèle.
...
- Modèles spécialisés par domaine : `LegalBERT`, `BioBERT`, `SciBERT`, etc. À ce jour, aucun modèle de vectorisation [*open-source*](#gloss-open-source) spécialisé santé-sécurité/BTP n'est librement disponible, ce qui constitue à la fois une limite et une opportunité ([*fine-tuning*](#gloss-fine-tuning) interne envisageable).

#### Dimensions de vectorisation : compromis qualité / coût {#sec4-1-2}
...
- le coût de stockage : un index de $N$ [*chunks*](#gloss-chunk) stocké en `float32` (format à virgule flottante standard sur 32 bits, soit 4 octets par composante) occupe $4 \cdot N \cdot d$ octets (ex. : 1 M [*chunks*](#gloss-chunk) en dim. 1024 ≈ 4 Go) ; des formats plus compacts (`float16`, `int8`, quantization via PQ) permettent de diviser ce coût par 2 à 8 au prix d'une légère perte de précision.
- la latence de recherche : croît linéairement avec $d$ pour la similarité, et indirectement via la taille de l'index ANN.

Les vectorisations "Matryoshka" ([*Matryoshka Representation Learning*](#gloss-matryoshka-representation-learning)) permettent de tronquer la dimension a posteriori avec une perte limitée, offrant un curseur qualité/coût ajustable sans réindexation complète.

#### Multilinguisme et adaptation au français technique {#sec4-1-3}
...
Le [*benchmark*](#gloss-benchmark) MTEB ([*Massive Text Embedding Benchmark*](#gloss-massive-text-embedding-benchmark)) fournit une comparaison standardisée entre modèles, mais il faut se rappeler que les performances MTEB ne se transposent pas mécaniquement à un domaine spécialisé [@Muennighoff2023MTEB]. Le [*benchmark*](#gloss-benchmark) BEIR a clairement montré la dégradation hors-domaine des [*retrievers*](#gloss-retriever) entraînés sur du web généraliste [@Thakur2021BEIR].

#### Évaluation intrinsèque vs extrinsèque {#sec4-1-4}

Deux niveaux d'évaluation se distinguent pour un modèle de vectorisation :

- Intrinsèque : qualité de la séparation paires positives / négatives (STS, [*retrieval*](#gloss-retrieval)@k sur jeux annotés, alignement avec jugements humains)
- Extrinsèque : impact sur la tâche aval (qualité de la réponse RAG finale)

Les deux ne coïncident pas toujours : une vectorisation de qualité qui remonte "les bons documents" peut tout de même conduire à une mauvaise réponse si le générateur exploite mal le contexte. C'est une raison supplémentaire pour évaluer les composants et la chaîne complète (cf. [Chapitre 5](#ch5)).

#### Critères de sélection en contexte industriel {#sec4-1-5}

En contexte d'entreprise, le choix d'un modèle de vectorisation ne se résume pas à un score sur un [*benchmark*](#gloss-benchmark). Une grille de décision multi-critères est nécessaire :
...
| [*Fine-tunabilité*](#gloss-fine-tuning) | Possibilité d'adapter le modèle au domaine santé-sécurité si nécessaire |

Table: Critères de sélection d'un modèle de vectorisation en contexte industriel.

Pour le prototype de ScribBERT, ces critères ont rapidement convergé vers `text-embedding-ada-002` d'OpenAI, principalement parce qu'il était déjà exposé dans le [*tenant*](#gloss-tenant) Azure OpenAI du groupe : pas de modèle à héberger, pas d'infrastructure GPU à monter, et la confidentialité couverte par les engagements déjà négociés pour les LLMs. Sa qualité intrinsèque n'est pas exceptionnelle (c'est un modèle déjà ancien) mais le [*benchmark*](#gloss-benchmark) de la [Partie III](#partie-3) montre qu'il fait jeu égal avec plusieurs alternatives [*open-source*](#gloss-open-source) sur le corpus considéré. La justification détaillée et les comparaisons chiffrées de latence et de MRR sont reportées au [§ 7.5.2](#sec7-5-2) et au [§ 8.2](#sec8-2).

### Le rôle du *chunking* et du prétraitement textuel {#sec4-2}

Le [*chunking*](#gloss-chunking) est probablement le sujet qui a demandé le plus de temps d'exploration : plusieurs jours, voire semaines, passés à [*benchmarker*](#gloss-benchmark) des algorithmes différents sur le même corpus avant de trancher. Il est souvent présenté comme un "détail d'ingestion" dans les tutoriels, mais c'est en réalité un choix de modélisation à part entière, dont les effets se propagent à toute la chaîne. La solution finalement retenue pour ScribBERT, est qu'une fois les PDF convertis en Markdown pour préserver la structure (titres, listes, tableaux), un [*chunking*](#gloss-chunking) par regex sur les marqueurs structurels donne le meilleur compromis : les référentiels du corpus partagent la même charte de mise en forme, donc une regex bien ciblée récupère proprement les titres, les numérotations, les paragraphes. C'est également plus rapide à exécuter et plus prévisible qu'un [*chunking*](#gloss-chunking) sémantique ou à longueur fixe pure.

\needspace{10\baselineskip}

#### Stratégies de *chunking* {#sec4-2-1}

Plusieurs approches existent, chacune avec ses compromis :

- [*Chunking*](#gloss-chunking) à taille fixe (nombre [*tokens*](#gloss-token) ou caractères) : simple, prévisible, mais aveugle à la structure. Risque majeur : couper une règle au milieu d'une phrase, ou séparer une condition de son exception.
- [*Chunking*](#gloss-chunking) récursif ([*recursive character text splitter*](#gloss-recursive-splitter)) : tente de découper d'abord sur des séparateurs "forts" (`\n\n`, `\n`, `. `, ` `) avant de tomber sur du caractère brut. Bon compromis par défaut, implémenté dans LangChain/LlamaIndex.
- [*Chunking*](#gloss-chunking) structurel : exploite la hiérarchie documentaire (titres, sections, listes, tableaux). Particulièrement adapté aux référentiels et normes qui ont une structure claire.
- [*Chunking*](#gloss-chunking) sémantique : utilise un modèle (souvent un modèle de vectorisation) pour détecter des ruptures de sujet et grouper les phrases sémantiquement proches. Plus coûteux en ingestion, gain variable.
- [*Chunking*](#gloss-chunking) sur mesure (regex / [*parser*](#gloss-parser) dédié) : pour des formats spécifiques (procédures avec format imposé, fiches sécurité), un [*parser*](#gloss-parser) dédié peut extraire des unités cohérentes (par exemple, une règle par paragraphe).
...
#### Taille des *chunks* et chevauchement {#sec4-2-2}

Deux paramètres clés interagissent :

- Taille du [*chunk*](#gloss-chunk) ($T$, en [*tokens*](#gloss-token)) : un chunk trop petit peut mener à une perte de contexte, de l'ambiguïté, perte de l'antécédent ("il", "cette règle"), à l'inverse d'un chunk trop grand qui amène de la dilution sémantique, une vectorisation moins discriminante, un contexte LLM potentiellement saturé.
- [*Overlap*](#gloss-overlap) ($O$, généralement 10-20% de $T$) : permet d'amortir les coupures malheureuses au prix d'une redondance dans l'index.

L'optimum dépend du type de question : les questions factuelles courtes tolèrent des [*chunks*](#gloss-chunk) petits, tandis que les questions procédurales ("comment faire X ?") requièrent souvent des [*chunks*](#gloss-chunk) plus larges qui capturent une séquence d'étapes. C'est ce genre de tensions qui a été observé lors du développement : en réduisant la taille des [*chunks*](#gloss-chunk), la précision augmentait sur certaines questions, mais la cohérence des réponses se dégradait sur d'autres. Un protocole rigoureux teste plusieurs configurations ($T \in \{256, 512, 1024\}$, $O \in \{0, 64, 128\}$) et mesure l'impact [*end-to-end*](#gloss-end-to-end); c'est ce qui est fait en [Partie III](#partie-3).

\needspace{10\baselineskip}

#### Préservation de la structure et des métadonnées {#sec4-2-3}

Un [*chunk*](#gloss-chunk) "brut" (texte seul) perd des informations critiques : section d'origine, niveau hiérarchique, type de document, date de validité, autorité émettrice. Or ces métadonnées :
...
#### Nettoyage et normalisation {#sec4-2-4}
...
- Normalisation : unification des guillemets, des espaces insécables, des tirets ; éventuellement passage en minuscules pour le [*sparse retrieval*](#gloss-sparse-retrieval) (mais pas pour les *vectorisations*, qui sont généralement sensibles à la casse).
- Conservation du formatage utile : listes à puces, numérotation hiérarchique, gras pour les termes-clés.

Un point souvent négligé : les tableaux et les schémas. Linéariser un tableau en texte brut détruit sa structure. Des stratégies plus avancées (extraction structurée, légendes générées par un VLM ([*Vision Language Model*](#gloss-vision-language-model)), tableaux convertis en markdown) peuvent être étudiées.

### Les stratégies de récupération {#sec4-3}

Une fois l'index constitué, la récupération comporte plusieurs leviers : choix de la similarité, hybridation sparse/dense, [*reranking*](#gloss-reranking), filtrage, expansion de requête, valeur de $k$.

#### similarité vectorielle par cosinus et alternatives {#sec4-3-1}

La similarité vectorielle par cosinus est la mesure par défaut pour comparer deux vectorisations :
...
#### Recherche hybride : combiner sparse et dense {#sec4-3-2}
...
Pour ScribBERT, l'hypothèse forte est qu'un utilisateur citant explicitement "PR-SST-042" doit retrouver ce document, ce que BM25 garantit mais qu'un dense pur peut manquer. Cette hypothèse sera testée en [Partie III](#partie-3). Empiriquement, en passant de `dense-k5` à `hybrid-k5` sur les questions réelles, le bénéfice est tangible : les bons documents remontent plus souvent en tête, et les cas où le bon document ne sort tout simplement pas se font plus rares. En revanche, aucun effet [*lost in the middle*](#gloss-lost-in-the-middle) significatif n'a été observé sur le top-5 de ScribBERT, le contexte restant suffisamment court pour que le LLM exploite chaque [*chunk*](#gloss-chunk).

#### *Reranking* par *cross-encoder* {#sec4-3-3}

Le [*reranking*](#gloss-reranking) consiste à appliquer un modèle plus précis (et plus coûteux) à un petit ensemble de candidats déjà récupérés. Les [*cross-encoders*](#gloss-cross-encoder) (ex. `ms-marco-MiniLM`, `bge-reranker-v2-m3`, `Cohere Rerank`) lisent conjointement la requête et le passage et produisent un score de pertinence [@NogueiraCho2019].

Chaîne de traitement typique :

1. Récupération initiale pour obtenir le top-100 des candidats (rapide, $O(\log N)$ sur HNSW),
2. [*Reranking*](#gloss-reranking) pour ne conserver que le top-10 (lent : 100 inférences [*cross-encoder*](#gloss-cross-encoder), $\sim$ 100-500 ms),
...
#### Filtrage par métadonnées {#sec4-3-4}
...
- Post-filtrage : récupérer puis filtrer (plus simple, mais peut vider le [*top-k*](#gloss-top-k) et empêcher d'autres documents pertinents de remonter).

Un filtrage trop strict peut éliminer les bons passages au même titre qu'un post-filtrage gaspille du calcul. Les bases vectorielles modernes (Qdrant, Weaviate, Pinecone) optimisent le pré-filtrage. ChromaDB, utilisé dans les premières instances de ScribBERT, supporte le pré-filtrage par métadonnées, bien qu'il soit davantage adapté au développement local et aux petits corpus.
...
#### Choix de $k$ : compromis rappel / bruit / coût {#sec4-3-5}
...
- $k$ trop grand : dilution, bruit, coûts élevés ([*tokens*](#gloss-token) consommés, latence), risque de [*lost in the middle*](#gloss-lost-in-the-middle) (le LLM ignore les passages au milieu du contexte).

Valeurs typiques : $k \in [3, 10]$ après [*reranking*](#gloss-reranking). La valeur optimale dépend du modèle de génération (les LLMs récents avec contexte long tolèrent mieux $k$ élevé, même si le phénomène [*lost in the middle*](#gloss-lost-in-the-middle) persiste y compris sur les contextes longs[@Liu2024LostMiddle]) et du type de question.

#### *Query expansion* et reformulation {#sec4-3-6}
...
- [*Multi-query*](#gloss-multi-query) : générer plusieurs reformulations de la requête, lancer plusieurs recherches, fusionner les résultats.
- [*Step-back prompting*](#gloss-step-back-prompting) : reformuler la requête en une question plus générale, qui peut mieux correspondre à des passages introductifs.
...
### La composante de génération {#sec4-4}
...
#### Choix du LLM {#sec4-4-1}

Les options se classent en trois catégories :

- LLMs propriétaires (API) : GPT-4 / GPT-4o (OpenAI), Claude 3.5/4 (Anthropic), Gemini (Google), Mistral Large. Excellente qualité, coût marginal par requête/[*token*](#gloss-token), dépendance à un fournisseur externe et contraintes de confidentialité.
- LLMs [*open-weights*](#gloss-open-source) auto-hébergés : Llama 3, Mistral / Mixtral, Qwen, DeepSeek, Gemma. Contrôle total des données, coût d'infrastructure (GPU).
...
Pour ScribBERT, l'absence d'infrastructure GPU chez Bouygues TP a rendu les modèles auto-hébergés peu viables : les tests en local sur un poste de développement standard n'ont permis de faire tourner que des modèles de petite taille, et même ceux-ci se sont révélés trop lents pour être exploitables. Le choix s'est porté sur un LLM propriétaire via Azure OpenAI, dans le contexte d'un contrat-cadre Bouygues Construction garantissant la confidentialité des données. Concrètement, pour le développement, le choix s'est porté sur `gpt-3.5-turbo` : il est bon marché, rapide, et la différence de qualité avec un GPT-4 / Claude / Mistral Large est amortie par la contextualisation RAG. Le déploiement via Azure OpenAI a aussi un atout opérationnel : aucune brique d'infrastructure à gérer en propre. Pour le passage en production, un modèle plus récent (GPT-4o ou équivalent) sera réévalué, notamment sur les questions justificatives où quelques limites qualitatives ont été perçues. 

#### Ingénierie de *prompt* {#sec4-4-2}

Le [*prompt*](#gloss-prompt) système est le contrat passé entre le développeur et le modèle. Un [*prompt*](#gloss-prompt) RAG contient généralement quatre éléments : les instructions système (rôle, contraintes, règles de comportement), la requête utilisateur, le contexte récupéré (passages formatés et numérotés), et le format de sortie attendu.

Dans la pratique, quelques principes font consensus. L'*ancrage explicite* est essentiel : il faut dire au modèle de ne répondre que sur la base des extraits fournis, et de l'indiquer clairement si l'information n'y figure pas. Les citations obligatoires ("cite chaque affirmation avec le numéro de la source") améliorent la traçabilité. Et surtout, il faut autoriser le modèle à dire "je ne sais pas". C'est contre-intuitif (l'utilisateur attend des réponses), mais c'est ce qui réduit le plus efficacement les hallucinations. (Un ou deux exemples ([*few-shot*](#gloss-few-shot)) de paires question/réponse peuvent aussi être ajoutés pour calibrer le style.)

#### Gestion de la fenêtre de contexte {#sec4-4-3}

Le budget de [*tokens*](#gloss-token) est une contrainte structurante. Avec 10 passages de 500 [*tokens*](#gloss-token) chacun et un modèle qui accepte 8k [*tokens*](#gloss-token) en contexte, il faut faire des choix. La stratégie la plus simple est la troncature (couper les passages les moins bien classés). Les [*chunks*](#gloss-chunk) longs peuvent aussi être compressés avant injection, ou le contexte rempli par ordre de pertinence jusqu'à un seuil. Les LLMs récents acceptent des contextes de 128k [*tokens*](#gloss-token) et plus, mais attention au phénomène [*lost in the middle*](#gloss-lost-in-the-middle) expliqué plus tôt : le modèle tend à moins bien exploiter les passages placés au milieu d'un contexte volumineux, ce qui peut fausser les réponses.

#### Paramètres de décodage {#sec4-4-4}
...
- Top-p / [*top-k*](#gloss-top-k) sampling : alternative à la température, plus rarement utilisée en RAG.
- Max [*tokens*](#gloss-token) : borne haute pour éviter les réponses interminables.
- Repetition / presence penalty : utile si le modèle "bégaie" sur des termes techniques.

Pour ScribBERT, une température faible est recommandée afin de garantir la stabilité des réponses (cf. [Chapitre 6](#ch6)).

#### Citations et traçabilité {#sec4-4-5}

La citation peut prendre plusieurs formes : [*inline*](#gloss-inline) ("Selon [1], le port du harnais est obligatoire dès 2 m"), en fin de réponse (liste des sources), ou avec reproduction littérale des passages clés.

L'important, au-delà du format, est que la traçabilité soit [*machine-vérifiable*](#gloss-machine-verifiable). Chaque citation doit pointer vers un identifiant de [*chunk*](#gloss-chunk) journalisé, lui-même relié au document d'origine. Sans cette chaîne, la traçabilité reste de surface, utile pour l'utilisateur mais insuffisante pour l'audit et pour la mesure de fidélité (cf. [Chapitre 6](#ch6)).

#### Garde-fous pour le contexte santé-sécurité {#sec4-4-6}
...
Ces garde-fous n'apparaissent pas spontanément : la première version du [*prompt*](#gloss-prompt) système de ScribBERT était beaucoup trop permissive. Sur des questions qui n'avaient rien à voir avec la santé-sécurité, le modèle sortait des recettes de cookies, répondait à partir de sa connaissance générale, et oubliait de citer ses sources. Les instructions ont dû être durcies progressivement : cadrer explicitement le domaine, exiger une citation, autoriser et même encourager le "je ne sais pas" lorsque le contexte ne contient pas l'information. C'est en partie ce qui a permis de comprendre que le [*prompt*](#gloss-prompt) fait autant partie de l'évaluation que le modèle ou la vectorisation.

### Synthèse des leviers et matrice d'expérimentation {#sec4-5}

L'ensemble des leviers présentés peut être résumé dans une matrice qui guidera la conception du protocole expérimental ([Chapitre 5](#ch5)) :

| Composant | Leviers principaux | Métriques affectées en priorité |
|------------------------------|--------------------------------------------------|---------------------------------|
| Vectorisation | Modèle, dimension, langue, [*fine-tuning*](#gloss-fine-tuning) | Recall@k, MRR, nDCG |
| [*Chunking*](#gloss-chunking) | Stratégie, taille, chevauchement, métadonnées | Recall@k, citabilité, fidélité |
| Récupération | Sparse / dense / hybride, filtres, $k$ | Recall@k, précision contexte |
| [*Reranking*](#gloss-reranking) | Présence, modèle, top-$n$ | Precision@k, fidélité |
...
| Génération ([*prompt*](#gloss-prompt)) | Instructions, [*few-shot*](#gloss-few-shot), format | Fidélité, format, refus contrôlé |
| Génération (décodage) | Température, max [*tokens*](#gloss-token) | Stabilité, longueur |

Table: Synthèse des leviers techniques d'un système RAG et métriques affectées.

L'expérimentation menée en [Partie III](#partie-3) ne pourra pas tester toutes les combinaisons (explosion combinatoire). Elle adoptera une approche OFAT ([*One-Factor-At-a-Time*](#gloss-one-factor-at-a-time)) sur un sous-ensemble de paramètres jugés les plus impactants, complétée par quelques expériences factorielles ciblées.

Le [Chapitre 5](#ch5) présente le protocole d'évaluation lui-même : jeux de test, métriques, conditions d'expérimentation.

```{=latex}
\newpage
```

## Construction d'un protocole d'évaluation {#ch5}

Le [Chapitre 4](#ch4) a inventorié les différents leviers actionnables. Reste la question fondamentale : **comment mesurer leur effet ?** Sans un protocole d'évaluation structuré, le tâtonnement décrit plus haut s'impose à nouveau : un paramètre est modifié, trois questions sont posées, et reste l'"impression" que c'est mieux ou moins bien, sans pouvoir trancher. C'est une leçon répétée à plusieurs reprises par Julien Larseneur dans l'équipe : la tentation initiale était de se fier à la fiabilité perçue, en posant quelques questions et en jugeant les réponses. Julien ne jure que par les métriques, et a détaillé les différentes familles (Recall, MRR, [*faithfulness*](#gloss-faithfulness)…), leurs limites individuelles, et l'intérêt d'en croiser plusieurs. Ce chapitre est en grande partie la formalisation de ces échanges.
...
Ce chapitre s'organise en cinq sections : les critères d'évaluation ([§ 5.1](#sec5-1)), les approches (automatique, humaine, hybride) ([§ 5.2](#sec5-2)), la construction du jeu de test ([§ 5.3](#sec5-3)), les conditions expérimentales ([§ 5.4](#sec5-4)), et les méthodes d'analyse ([§ 5.5](#sec5-5)).

### Cinq dimensions pour mesurer la fiabilité {#sec5-1}

Le [Chapitre 3.3](#sec3-3) a défini la fiabilité opérationnellement comme la conjonction de cinq propriétés. Plutôt que de dresser une liste plate de métriques, j'organise ici l'évaluation autour de ces cinq dimensions : pour chacune, le type d'échec à détecter est précisé, puis les métriques candidates, en privilégiant celles qui sont effectivement utilisables dans un cadre industriel.

#### Dimension 1 - Pertinence de la récupération {#sec5-1-1}
...
- MRR ([*Mean Reciprocal Rank*](#gloss-mean-reciprocal-rank)) : le premier passage pertinent est-il bien classé en tête ? C'est la bonne métrique lorsqu'un passage décisif est principalement attendu.
...
#### Dimension 2 - Fidélité aux sources (*faithfulness*) {#sec5-1-2}
...
- Faithfulness (RAGAS) : la réponse est décomposée en propositions atomiques ("le port du harnais est obligatoire dès 2 m"), et chacune est vérifiée contre le contexte par un [*LLM-juge*](#gloss-llm-as-judge). Le score final est la proportion de propositions supportées.
...
- [*Citation faithfulness*](#gloss-citation-faithfulness) : lorsque la réponse cite un passage explicitement, ce passage supporte-t-il réellement l'affirmation ? C'est une vérification de cohérence entre la citation et le contenu.
- Hallucination rate : simplement le taux de propositions non supportées (= 1 − [*faithfulness*](#gloss-faithfulness)).

À ces métriques génériques peut s'ajouter, en contexte santé-sécurité, une mesure plus spécifique : la préservation des modalités : la réponse respecte-t-elle les niveaux d'obligation des sources ("doit" vs "peut" vs "il est recommandé de") ? Cette dimension est difficile à automatiser de façon fiable et nécessite souvent une vérification humaine ou un [*LLM-juge*](#gloss-llm-as-judge) avec des instructions très précises à ce sujet.

#### Dimension 3 - Pertinence et complétude de la réponse {#sec5-1-3}
...
- Answer relevance (RAGAS) : un [*LLM-juge*](#gloss-llm-as-judge) génère plusieurs questions hypothétiques à partir de la réponse produite, puis mesure si elles ressemblent à la question originale. Une réponse hors-sujet ou vague produira des questions hypothétiques éloignées.
...
- Respect du format : si le [*prompt*](#gloss-prompt) demande une check-list numérotée, le modèle l'a-t-il bien produite ?

#### Dimension 4 - Stabilité et répétabilité {#sec5-1-4}

*Si la même question est rejouée, la réponse est-elle cohérente ?*

Un système peut obtenir de bons scores en moyenne tout en produisant des réponses très variables d'une exécution à l'autre. Cette dimension, traitée en détail au [Chapitre 6](#ch6), mesure la variance des réponses plutôt que leur qualité moyenne. Elle conditionne également la robustesse statistique de toutes les comparaisons du protocole : si la variabilité intra-configuration est élevée, comparer deux configurations sur une seule exécution par question n'a pas de sens.

#### Dimension 5 - Traçabilité et auditabilité {#sec5-1-5}
...
- [*Citation correctness*](#gloss-citation-correctness) : les passages cités existent-ils, sont-ils pertinents, et supportent-ils réellement l'affirmation ?
- [*Citation completeness*](#gloss-citation-completeness) : toutes les affirmations qui devraient être sourcées le sont-elles ?
...
Ces métriques ne sont utiles que si le [*prompt*](#gloss-prompt) impose un format de citation [*machine-vérifiable*](#gloss-machine-verifiable) (identifiants de [*chunks*](#gloss-chunk), pas juste des titres de documents).

#### Coût opérationnel {#sec5-1-6}
...
- Latence de la chaîne complète (récupération + [*reranking*](#gloss-reranking) + génération). En pratique, un percentile P95 est plus significatif que la moyenne pour mesurer l'expérience utilisateur réelle.
...
### Approches d'évaluation : automatique, humaine, hybride {#sec5-2}

#### Évaluation automatique {#sec5-2-1}

Les métriques automatiques se classent en trois familles :

- Lexicales (BLEU, ROUGE, METEOR, [*exact match*](#gloss-exact-match)) : peu adaptées à la QA générative car elles pénalisent la paraphrase légitime. Utiles uniquement pour des réponses très courtes et factuelles.
- Vectorielles (BERTScore, BLEURT) : capturent mieux la similarité sémantique. En limitation, ces métriques pourraient amener à juger comme "proches" deux réponses dont l'une contiendrait une erreur factuelle subtile [@Zhang2020BERTScore; @Sellam2020BLEURT].
...
Avantages : passage à l'échelle (millions de requêtes), reproductibilité (à seed et [*prompt*](#gloss-prompt) fixés), coût marginal réduit.

Limites :

- corrélation imparfaite avec le jugement humain expert (surtout en domaine spécialisé comme dans notre cas) ;
- biais du [*LLM-juge*](#gloss-llm-as-judge) (préférence pour les réponses verbeuses, biais de longueur, biais de formatage) ;
...
- Utiliser un [*LLM-juge*](#gloss-llm-as-judge) différent du générateur évalué.
- Si possible, "calibrer" le [*LLM-juge*](#gloss-llm-as-judge) sur un échantillon annoté humainement (quelques exemples).
- *Journaliser* les justifications du juge, pas seulement le score.
- Mesurer la stabilité du juge lui-même (même [*prompt*](#gloss-prompt), $n$ exécutions).

#### Évaluation humaine {#sec5-2-2}

Constitue le [*gold standard*](#gloss-gold-standard), particulièrement pour les dimensions difficiles à automatiser (modalités, sécurité, exceptions).
...
#### Approche hybride : évaluation automatique préliminaire et validation humaine ciblée {#sec5-2-3}
...
3. Calibration : l'échantillon annoté manuellement sert à corriger les biais identifiés dans le [*LLM-juge*](#gloss-llm-as-judge) et à mieux interpréter ses scores sur le reste du jeu de test.
...
### Construction du jeu de test {#sec5-3}

La qualité du jeu de test conditionne la validité de toute l'évaluation. Cette section décrit la démarche méthodologique générique. L'instanciation pour ScribBERT figurera en [Partie III](#partie-3).

#### Sources des questions {#sec5-3-1}
...
3. Questions générées par LLM à partir des documents : pour chaque [*chunk*](#gloss-chunk) pertinent, un LLM génère une question dont la réponse est dans le [*chunk*](#gloss-chunk). Permet une couverture exhaustive du corpus mais introduit un biais (questions trop bien formées).
...
#### Typologie des questions {#sec5-3-2}

Pour une évaluation par un protocole diagnostique, il convient de stratifier le jeu de test selon plusieurs axes.
...
- Réponse de référence rédigée par un expert (idéalement validée par un second expert, mais [*time-consuming*](#gloss-time-consuming)).
- Passages de référence : identifiants des [*chunks*](#gloss-chunk) contenant l'information nécessaire et suffisante.
...
#### Volume et représentativité {#sec5-3-4}
...
- le corpus de référence (documents, [*chunks*](#gloss-chunk), vectorisations) : un jeu de test n'a de sens que pour une version donnée du corpus,
- les annotations (qui, quand, sur quelle base).

### Conditions expérimentales et reproductibilité {#sec5-4}

#### Isolation des facteurs {#sec5-4-1}

Étant donné l'explosion combinatoire des leviers ([Ch. 4](#ch4)), deux stratégies sont typiquement adoptées :

- OFAT ([*One-Factor-At-a-Time*](#gloss-one-factor-at-a-time)) : faire varier un paramètre à la fois autour d'une configuration de référence. Simple, interprétable, mais ne capture pas les interactions.
...
### Méthodes d'analyse {#sec5-5}

#### Statistiques descriptives {#sec5-5-1}
...
#### Tests de significativité {#sec5-5-2}
...
#### Stratification et analyses par sous-groupe {#sec5-5-3}
...
#### Analyse d'erreurs typologique {#sec5-5-4}
...
|---------------------------------------|--------------------------------------------|--------------------------------------|
| Échec de récupération | Aucun passage pertinent dans le top-$k$ | *Vectorisation* / [*chunking*](#gloss-chunking) / $k$ |
| Bruit de récupération | Passages tentants mais non applicables | *Vectorisation* / [*reranking*](#gloss-reranking) |
| Hallucination factuelle | Affirmation non supportée | Génération / [*prompt*](#gloss-prompt) |
| Omission d'exception | Règle correcte mais condition oubliée | Génération / contexte tronqué |
| Inversion de modalité | "doit" devenu "peut" | Génération / [*prompt*](#gloss-prompt) |
| Contradiction silencieuse | Sources divergentes non signalées | [*Prompt*](#gloss-prompt) / corpus |
| Refus à tort | Refuse alors que l'info est dans le contexte | [*Prompt*](#gloss-prompt) / seuils |
| Réponse hors-périmètre acceptée | Aurait dû refuser | [*Prompt*](#gloss-prompt) / garde-fous |

Table: Typologie générique des erreurs d'un système RAG et localisation probable.

Cette typologie sert de grille pour l'analyse qualitative en [Partie III](#partie-3) et oriente les améliorations.
...
Le [Chapitre 6](#ch6) approfondit la dimension stabilité, qui mérite un traitement spécifique car elle est sous-traitée par les [*frameworks*](#gloss-framework) usuels et particulièrement critique pour un système RAG en production sur un sujet sensible.

```{=latex}
\newpage
```

## Évaluation de la stabilité et de la répétabilité {#ch6}

### Pourquoi la stabilité est une dimension distincte de la fiabilité {#sec6-1}
...
1. Stochasticité de la génération : à température > 0, le LLM échantillonne à chaque [*token*](#gloss-token), conduisant à des réponses différentes pour une même entrée.
...
3. Sensibilité au [*prompt*](#gloss-prompt) et à la formulation : une question tournée différemment peut modifier le top-$k$ retourné et donc la réponse.
...
Cette dimension est aussi un enjeu méthodologique : si la variance au sein d'une même configuration est élevée, comparer deux configurations sur une exécution unique n'a pas de sens, le bruit de mesure dépasse l'effet à mesurer. L'évaluation de la stabilité conditionne donc la robustesse statistique des comparaisons du [Chapitre 5](#ch5).

### Sources de variance dans un RAG {#sec6-2}

Ces sources de variance n'ont pas pu être testées systématiquement sur ScribBERT dans le cadre de ce travail (cf. limites, [Ch. 11](#ch11)). La cartographie ci-dessous reste donc en partie théorique, fondée sur la littérature et sur quelques observations ponctuelles pendant le développement.

Côté génération, la variance vient d'abord de l'échantillonnage stochastique (température, [*top-p*](#gloss-top-p)) qui agit sur la diversité lexicale et, à température élevée, sur le contenu factuel lui-même. Même à température 0, le non-déterminisme persiste sur les LLM propriétaires : parallélisme GPU et *traitement par lots* variable empêchent un déterminisme strict, et il faut s'appuyer sur des paramètres dédiés (`seed`, identifiant `system_fingerprint` côté OpenAI / Azure OpenAI) pour tracer ce qui est effectivement reproductible. S'y ajoutent les choix de format : un même contenu peut être rendu en puces ou en phrases, ce qui fausse toute comparaison textuelle brute. Les sources de variance côté récupération (approximation ANN, égalités de scores, concurrence sur index distribué) ont déjà été décrites en [§ 1.4](#sec1-4), [§ 3.2](#sec3-2) et [§ 6.1](#sec6-1), et ne sont pas reprises ici.
...
Enfin, à plus long terme, une dérive temporelle s'installe : mise à jour silencieuse des modèles propriétaires (un `gpt-4o-2024-08-06` peut être déprécié et remplacé), évolution du corpus (ajouts, retraits, révisions de procédures), et dérive de l'index dès que la stratégie de [*chunking*](#gloss-chunking) ou de vectorisation est modifiée.

### Métriques de stabilité {#sec6-3}

Le cas le plus simple est la stabilité inter-runs : à requête et configuration constantes, le système est exécuté $n$ fois (typiquement $n \in [5, 20]$) et le recouvrement des sorties est mesuré. Côté récupération, un Stability@retrieval est calculé comme indice de Jaccard moyen des ensembles de [*chunks*](#gloss-chunk) récupérés entre paires de runs. Le Jaccard mesure le recouvrement entre deux ensembles, défini comme le rapport entre la taille de leur intersection et celle de leur union : $\mathrm{J}(A_i, A_j) = |A_i \cap A_j| / |A_i \cup A_j|$. Il vaut 1 si les deux runs retournent exactement les mêmes [*chunks*](#gloss-chunk), 0 s'ils sont disjoints. La même mesure peut être restreinte aux [*chunks*](#gloss-chunk) effectivement cités dans la réponse (et non simplement récupérés), ce qui est souvent plus informatif sur la fidélité perçue. Côté génération, un BERTScore moyen entre paires de réponses donne une stabilité sémantique, complétée par l'écart-type inter-runs des métriques de qualité ([*faithfulness*](#gloss-faithfulness), Recall@k…) et par un taux de basculement : taux de questions pour lesquelles le verdict (réponse acceptable / inacceptable) change entre runs. Dans le cas binaire correct/incorrect, le résumé se fait par la proportion de runs corrects ; les questions dont ce taux se situe entre 30% et 70% sont flaguées comme "instables".

Cette stabilité à requête identique ne suffit pas : un système peut être stable inter-runs et fragile aux paraphrases. Pour chaque requête, $m$ reformulations sont donc générées et les mêmes métriques sont appliquées entre la requête originale et ses variantes, en confiant à un [*LLM-juge*](#gloss-llm-as-judge) la vérification que les réponses véhiculent bien la même information factuelle au-delà des différences de surface. Dans le même esprit, la robustesse à l'ordre des passages (sensibilité au [*lost in the middle*](#gloss-lost-in-the-middle)) peut être testée en permutant l'ordre des [*chunks*](#gloss-chunk) injectés dans le contexte : un système robuste produit des réponses sémantiquement équivalentes quel que soit l'ordre.

### Sensibilité aux paramètres et aux variations adverses {#sec6-4}
...
- Bruit dans le contexte : ajout de [*chunks*](#gloss-chunk) non pertinents pour mesurer la résistance à la dilution.
...
### Synthèse de la Partie II {#sec6-7}

Les chapitres 4 à 6 ont défini un cadre méthodologique complet :

- [Ch. 4](#ch4) a inventorié les leviers techniques actionnables (vectorisation, [*chunking*](#gloss-chunking), récupération, génération) avec leurs compromis ;
- [Ch. 5](#ch5) a structuré le protocole d'évaluation autour des cinq dimensions de la fiabilité, avec des approches automatiques, humaines et hybrides ;
- [Ch. 6](#ch6) a approfondi la dimension stabilité aussi bien perçue que statistique, sous-traitée mais critique pour un déploiement en production.

La [Partie III](#partie-3) instancie ce cadre sur ScribBERT : architecture déployée ([Ch. 7](#ch7)), résultats expérimentaux ([Ch. 8](#ch8)-[9](#ch9)), enjeux éthiques, réglementaires et industriels ([Ch. 10](#ch10)) et discussion ([Ch. 11](#ch11)).
...
Un point de vocabulaire est à clarifier d'emblée : trois chaînes de traitement distinctes coexistent dans la suite, chacune servant une finalité propre, et chacune désignée par un nom différent pour éviter toute ambiguïté de lecture entre les tableaux. La première est la **configuration POC** (`markdown-1200-50` + `ada-002` + dense + top-$k$ = 10 avec seuil de similarité 0,83), décrite au Ch. 7.6 : c'est celle qui tourne effectivement en production interne. La deuxième est la **configuration de stabilité** (`markdown-1200-50` + `ada-002` + `dense-k5-thresh` + `azure-gpt35`), utilisée au § 8.4 : elle reprend le couple *chunker* / *vectorisation* du POC, mais réduit le top-$k$ à 5 pour rester alignée sur la variante effectivement *benchmarkée* parmi les six du § 8.1.1. La troisième est la **configuration de référence** (`recursive-512-64` + `ada-002` + `hybrid-k5` + `azure-gpt35`), retenue à partir du § 9.1 comme cible pour l'analyse d'erreurs : il s'agit de la meilleure chaîne identifiée par la campagne de génération, candidate à la prochaine itération du POC, pas de celle actuellement déployée. Ce choix est délibéré : analyser les erreurs sur la configuration en route vers la production est plus actionnable qu'analyser celles d'une configuration appelée à évoluer. Sauf mention explicite, l'expression « configuration de référence » désigne dans la suite cette troisième chaîne.

La structure est la suivante :

- [Ch. 7](#ch7) : architecture déployée et choix techniques.
- [Ch. 8](#ch8) : résultats quantitatifs.
- [Ch. 9](#ch9) : analyse qualitative et étude d'erreurs.
- [Ch. 10](#ch10) : enjeux éthiques, réglementaires et industriels.
- [Ch. 11](#ch11) : discussion et perspectives.

## Mise en œuvre du système RAG ScribBERT {#ch7}

### Contexte et historique du projet {#sec7-1}
...
1. Phase de POC (*Proof of Concept*) : prototype rapide visant à valider la faisabilité technique, l'utilité réelle de la solution et son appropriation par les utilisateurs métier.
2. Phase exploratoire / industrialisation : [*benchmark*](#gloss-benchmark) des composants, construction d'une architecture, préparation à la mise en production (engagement, cadrage,...).
...
### Architecture déployée {#sec7-2}

#### Vue d'ensemble {#sec7-2-1}

ScribBERT suit l'architecture RAG classique décrite au [Ch. 2.3](#sec2-3), instanciée comme suit :
...
| Orchestration RAG | LangChain | Maturité, intégrations préexistantes ([*loaders*](#gloss-loader), [*splitters*](#gloss-splitter), [*retrievers*](#gloss-retriever), chains) ; permet de basculer à chaud entre différents fournisseurs (hébergement local, cloud) |
| Calcul tensoriel | PyTorch (via `sentence-transformers` et `transformers`) | Standard de fait pour l'inférence des modèles de vectorisation et des LLMs [*open-source*](#gloss-open-source) ; bonne intégration GPU sur le [*cluster*](#gloss-cluster) |
| Base vectorielle | ChromaDB | Léger, facile à intégrer, gestion native des métadonnées et du filtrage, déploiement local sans dépendance cloud |
...
| Hébergement (POC) | *Cluster* Kubernetes local au LabTP (équipe Lab TP Innovation) pour la chaîne applicative (UI, *back-end*, base vectorielle ChromaDB) ; LLM et modèle de vectorisation déportés sur l'API Azure OpenAI (cf. § 7.5) | Maîtrise interne du corpus indexé, de la base vectorielle et des flux applicatifs ; les seuls flux sortants (requête utilisateur, passages cités à `ada-002` et `gpt-3.5-turbo`) transitent vers Azure OpenAI sous contrat-cadre Bouygues Construction garantissant la confidentialité (pas de réutilisation pour l'entraînement, résidence des données encadrée) |

Table: Pile technique ScribBERT durant les tests.

Ce choix d'une *stack* majoritairement *open-source* et auto-hébergée répond aux contraintes de confidentialité et d'indépendance vis-à-vis de fournisseurs externes pour une éventuelle exploitation à long terme. La seule dépendance externe résiduelle est l'API Azure OpenAI, mobilisée pour le LLM (`gpt-3.5-turbo`) et le modèle de vectorisation (`ada-002`) : elle est encadrée par le contrat-cadre Bouygues Construction (confidentialité, non-réutilisation pour l'entraînement, résidence des données), et une bascule vers une chaîne 100 % auto-hébergée (Mistral local + vectorisation *open-source*) a par ailleurs été évaluée comme alternative souveraine, avec les compromis chiffrés au § 7.5.2 et § 8.3.

#### Chaîne de traitement d'ingestion {#sec7-2-3}

La chaîne de traitement d'ingestion transforme un PDF source en [*chunks*](#gloss-chunk) indexés. Étapes :

1. Extraction : conversion du PDF en Markdown via fitz (PyMuPDF), ce qui permet de conserver au mieux la mise en forme (titres, listes, tableaux), tout en disposant de texte facile à [*parser*](#gloss-parser).
2. Nettoyage : suppression des en-têtes/pieds de page répétitifs, normalisation des caractères spéciaux.
3. [*Chunking*](#gloss-chunking) : découpage par regex sur les marqueurs structurels (titres Markdown `#`, `##`, séparateurs de paragraphes), avec contrainte de taille cible (~1200 [*tokens*](#gloss-token)) et chevauchement (~50 [*tokens*](#gloss-token)). Détails en [§ 7.4](#sec7-4).
4. Enrichissement métadonnées : ajout pour chaque [*chunk*](#gloss-chunk) de : `nom_document`, `entité_émettrice`, `langue`, `position_dans_doc`.
5. Vectorisation : calcul vectoriel via le modèle retenu ([§ 7.5](#sec7-5)).
6. Indexation : insertion dans ChromaDB avec la collection appropriée.

L'étape 1 est actuellement la plus fragile : les PDFs santé-sécurité contiennent souvent des tableaux et des schémas, ignorés ou linéarisés à la volée comme du texte dans les débuts de ScribBERT. Pour la version production, une chaîne image-to-text est en cours d'étude : un modèle multimodal génère une description textuelle de chaque image/tableau, conserve le lien vers l'image originale, et l'injecte comme un [*chunk*](#gloss-chunk) enrichi. Cette piste sera évaluée séparément dans le [Ch. 11](#ch11) qui traite des perspectives d'évolution.

#### Interface et expérience utilisateur {#sec7-2-4}
...
- un avertissement permanent rappelant que la réponse n'engage pas la responsabilité du système et que l'utilisateur reste tenu de vérifier les sources citées (cf. [§ 9.3](#sec9-3)).

### Description du corpus {#sec7-3}
...
Cette taille reste modeste à l'échelle d'un [*benchmark*](#gloss-benchmark) IR (*Information Retrieval*) (BEIR utilise des corpus de 10⁵ à 10⁶ documents), mais elle est représentative d'un cas d'usage d'entreprise : un corpus expert, multilingue, à forte densité informationnelle, où chaque document compte. Le défi n'est pas le passage à l'échelle, mais plutôt la qualité de la récupération et de la génération sur un domaine spécialisé.
...
### Choix de *chunking* et prétraitement {#sec7-4}

Conformément à la grille du [Ch. 4.2](#sec4-2), la stratégie retenue est un [*chunking*](#gloss-chunking) structurel sur mesure, justifié comme suit :
...
- la cible de taille est d'environ 1 200 [*tokens*](#gloss-token) par [*chunk*](#gloss-chunk) (variante `markdown-1200-50` du [*benchmark*](#gloss-benchmark), retenue en POC), ce qui correspond empiriquement à un compromis entre :
  - assez large pour contenir une règle complète avec ses conditions et ses exceptions (cf. risque d'omission identifié au [Ch. 5.5.4](#sec5-5-4)),
  - assez petit pour rester discriminant à la vectorisation et économique en [*tokens*](#gloss-token) lors de l'injection dans le contexte LLM ;
  - le [*benchmark*](#gloss-benchmark) systématique ([§ 8.2](#sec8-2)) montre que les [*chunkings*](#gloss-chunking) 1024 [*tokens*](#gloss-token) dominent en MRR, ce qui valide a posteriori l'ordre de grandeur retenu ;
- le chevauchement est de ~50 [*tokens*](#gloss-token), soit une valeur faible (≈ 4%), qui suffit à amortir des coupures malheureuses sans gonfler significativement l'index ;
- une fenêtre contexte est ajoutée à la récupération : pour chaque [*chunk*](#gloss-chunk) retourné par la récupération, les [*chunks*](#gloss-chunk) $n-1$ et $n+1$ sont automatiquement adjoints avant injection dans le contexte LLM. Cette mécanique compense un chevauchement faible et restaure le contexte amont/aval, particulièrement utile pour les références anaphoriques ("cette règle", "les EPI mentionnés") et pour la cohérence procédurale.

Les métadonnées attachées à chaque [*chunk*](#gloss-chunk) sont actuellement : `nom_document`, `entité_émettrice`, `langue`, `date du document`.

### Choix de vectorisation et de LLM {#sec7-5}
...
#### Phase exploratoire : *benchmark* systématique {#sec7-5-2}

La phase exploratoire a consisté en un [*benchmark*](#gloss-benchmark) récupération portant sur 18 modèles de vectorisation testés, dont 2 modèles (`gte-qwen2-7b` et `nv-embed-v2`) ont dû être écartés intégralement faute d'avoir pu être initialisés dans l'environnement local (incompatibilités avec la version de `transformers` installée). Les 16 modèles retenus, croisés avec 9 stratégies de [*chunking*](#gloss-chunking) et 6 variantes de récupération, donnent un plan de 16 × 9 × 6 = 864 cellules, exécuté de bout en bout sur les 50 questions du jeu de test interne ([§ 8.1.2](#sec8-1-2)). Il a été constaté en pratique que mettre à jour la librairie pour débloquer les deux modèles écartés cassait à l'inverse plusieurs autres modèles déjà fonctionnels : un arbitrage propre sur le couple `transformers` / `sentence-transformers` (et plus largement sur l'épinglage des versions de l'écosystème HuggingFace) sera nécessaire à terme, mais la voie la plus rapide a été privilégiée ici, consistant à figer un environnement compatible avec le plus grand nombre de modèles disponibles. Sur les 864 cellules retenues, 750 ont produit des résultats exploitables, les autres correspondant à des échecs partiels résiduels sur quelques variantes (notamment `jina-v2-base-en` et `jina-v3`).

Les 16 modèles retenus couvrent les familles définies au [Ch. 4.1.1](#sec4-1-1) :

- propriétaires via API : `ada-002`, `embed-3-large` (OpenAI) ;
- multilingues [*open-source*](#gloss-open-source) orientés récupération : `e5-small-ml`, `e5-base-ml`, `e5-large-ml`, `bge-m3`, `jina-v3`, `nomic-v2`, `granite-311m-ml`, `qwen3-embed-8b` ;
- généralistes [*open-source*](#gloss-open-source) : `minilm-l6`, `mpnet-base`, `jina-v2-base-en` ;
- francophones / bilingues spécialisés : `camembert-large`, `solon-large`, `bilingual-fr-en`.

Les six variantes de récupération testées sont : `dense-k5`, `dense-k10`, `dense-k5-thresh` (seuil de similarité), `dense-k5-neigh` (voisinage n−1/n+1), `hybrid-k5` (dense + BM25, fusion RRF) et `dense-k20-rerank5` ([*reranking*](#gloss-reranking) [*cross-encoder*](#gloss-cross-encoder) BGE).

Pour chaque configuration, les métriques de récupération du [Ch. 5.1.1](#sec5-1-1) ont été collectées (Hit@k, Recall@k, Precision@k, MRR, nDCG@k pour $k\in\{1,3,5,10\}$, plus latence par requête). Une seconde campagne génération a ensuite été lancée sur cinq configurations sélectionnées comme représentatives (trois côté Azure avec évaluation RAGAS complète, deux côté local avec Mistral-7B auto-hébergé), discutées au [§ 8.3](#sec8-3).

Au vu des résultats consolidés, le modèle de vectorisation retenu pour la configuration dense de référence est `ada-002` (Azure OpenAI), au coude à coude avec `embed-3-large`, `nomic-v2` et `qwen3-embed-8b` sur la MRR moyenne (cf. [§ 8.2](#sec8-2)). Le choix repose sur trois éléments pratiques.

Intégration et infrastructure. `ada-002` est déjà déployé et disponible dans le [*tenant*](#gloss-tenant) Azure de Bouygues Construction, ce qui supprime à la fois le coût d'hébergement GPU et le délai de mise en place d'une architecture dédiée côté Bouygues.
...
Latence acceptable pour un usage interactif. La métrique `latency_s` enregistrée correspond au temps mesuré côté Python autour de l'appel de vectorisation seul, c'est-à-dire l'intervalle entre l'envoi de la requête depuis la machine de développement vers l'[*endpoint*](#gloss-endpoint) Azure et la réception du vecteur en retour (aller-retour réseau inclus, hors récupération ChromaDB et hors appel LLM). Les mesures ont été conduites sur une machine Dell Pro Max GB10 (CPU 20 cœurs Arm, GPU Blackwell intégré, 128 Go de LPDDR5X unifiée), connectée en Ethernet filaire sur une liaison fibre 1 Gbit/s symétrique. Sur cette configuration, la latence médiane observée est d'environ 80 ms par requête pour `ada-002`, contre 5-25 ms pour la plupart des modèles [*open-source*](#gloss-open-source) légers exécutés localement sur GPU (`minilm-l6`, `e5-*`, `mpnet-base`, `bge-m3`, `nomic-v2`, `solon-large`, etc.), environ 260 ms pour `qwen3-embed-8b` et jusqu'à environ 3 300 ms pour `embed-3-large`. Autrement dit, `ada-002` n'est pas le plus rapide en latence brute de vectorisation, mais il évite l'hébergement GPU et reste largement en-dessous des autres modèles propriétaires et des très gros [*open-source*](#gloss-open-source) ; sa latence est donc négligeable devant celle de la génération LLM (environ 5 000 ms côté Azure, cf. ci-dessous).

Concernant les modèles de génération de texte, les cinq runs disponibles (§ 8.3) confirment que `azure-gpt35` est suffisant pour la phase exploratoire : *faithfulness* RAGAS comprise entre 0,72 et 0,77, *answer relevancy* entre 0,72 et 0,76, pour une latence de génération médiane d'environ 5 s. Les deux runs Mistral-7B local atteignent des temps de génération de l'ordre de 36 à 38 s par question malgré l'accélération GPU et une *faithfulness* en retrait (0,61 à 0,68 selon la combinaison *chunker* et modèle de vectorisation), ce qui les disqualifie en tant que LLM principal du POC, mais les conserve comme piste de repli souverain pour des environnements sans accès Azure (typiquement des chantiers comme des sites militaires ou des installations nucléaires) sur lesquels Bouygues Construction doit parfois opérer en infrastructure totalement isolée d'Internet, ce qui exclut d'entrée de jeu tout appel API externe et impose une chaîne RAG 100% auto-hébergée. En pratique, la cible pour la mise en production est d'utiliser des modèles quasi état-de-l'art côté OpenAI (typiquement `gpt-4o` ou `gpt-4.1`), Anthropic (`claude-sonnet-4` / `claude-opus-4`) et Mistral (`mistral-large` ou successeurs), selon ce qui sera disponible et les coûts de run associés. Cette projection reste à valider expérimentalement sur le corpus ScribBERT : les modèles Anthropic mentionnés sont des modèles récents (2025) dont les performances effectives sur ce corpus n'ont pas été mesurées à la date de rédaction, et la sélection définitive sera arbitrée par un *benchmark* dédié lors de l'industrialisation. Le gain attendu porte principalement sur la *faithfulness* et l'*answer relevancy*, à reventiler face au coût par requête.

### Configuration de la récupération {#sec7-6}

| Paramètre | Valeur retenue | Renvoi théorique |
|--------------------------------------------------|---------------------------------------------------------------------------------------|------------------------------------------------------------|
| Type de récupération | Dense pur | [Ch. 4.3.2](#sec4-3-2) (hybridation BM25+dense identifiée comme amélioration) |
| Modèle de vectorisation | text-embedding-ada-002 (déploiement Azure configuré) | [Ch. 4.1](#sec4-1), [§ 7.5](#sec7-5) |
| Similarité | Cosinus (espace HNSW configuré sur cosine dans ChromaDB) | [Ch. 4.3.1](#sec4-3-1) |
| Top-$k$ | 10 | [Ch. 4.3.5](#sec4-3-5) |
| Filtre par score | Filtrage par distance avec seuil maximal 0,17. Les [*chunks*](#gloss-chunk) avec distance >= 0,17 sont écartés | [Ch. 5.1.2](#sec5-1-2) (lutte anti-hallucination par ancrage faible) |
| [*Reranking*](#gloss-reranking) | Inexistant sur le POC, mais présent dans le cahier des charges pour l'industrialisation | [Ch. 4.3.3](#sec4-3-3) |
| Filtres métadonnées | Filtrage récupération actif sur `doc_name` (liste d’inclusions) ; les filtres `client`/`langue` sont d'abord traduits en [*mapping*](#gloss-mapping) de `doc_name` concernés, puis appliqués sur ces `doc_name` directement | [Ch. 4.3.4](#sec4-3-4) |
| Contextualisation | Contextualisation par concaténation du [*chunk*](#gloss-chunk) précédent, courant et suivant (n-1, n, n+1) lors de l’indexation, avec garde-fou sur ruptures de chapitre | [§ 7.4](#sec7-4) |

Table: Configuration de la récupération ScribBERT (POC).

Le choix d'un dense pur s'explique par la simplicité d'implémentation au POC et par une qualité jugée suffisante en évaluation interne (cf. [Ch. 4.3.2](#sec4-3-2)). L'hybridation sparse+dense (BM25 + vectorisations) reste toutefois une amélioration prioritaire, particulièrement pertinente pour les requêtes contenant des correspondances exactes (numéros de procédure, codes EPI, références normatives), mieux captées par une composante lexicale.

Le filtrage actuellement implémenté repose sur une distance maximale. En pratique, les [*chunks*](#gloss-chunk) au-delà du seuil sont exclus (cf. [Ch. 5.1.2](#sec5-1-2)). En revanche, le refus contrôlé strict n'est pas totalement verrouillé dans la version actuelle : quand aucun [*chunk*](#gloss-chunk) pertinent n'est retenu, un message de contexte indique qu'aucun document n'a été trouvé, mais le modèle peut encore s'appuyer sur l'historique de conversation, ce qui rappelle la nécessité d'un garde-fou plus strict (cf. [Ch. 4.4.6](#sec4-4-6)).

\needspace{6\baselineskip}

### Configuration de la génération {#sec7-7}

[*Prompt*](#gloss-prompt) : structure conforme aux principes énoncés au [Ch. 4.4.2](#sec4-4-2) :
...
prompt = (
    ...
    f"(sans modifier ni reformuler le nom, respecte la casse, n'ajoute
    pas d'accents). "
...
- Température : 0,05 (réglage effectif de la route de génération principale, cohérent avec la recommandation de stabilité formulée plus haut en [Ch. 4.4.4](#sec4-4-4) et au [Ch. 6](#ch6)) ;
- Max [*tokens*](#gloss-token) : non fixé explicitement, pas de plafond applicatif dédié dans cette couche ;
...
### Synthèse des choix et limites assumées {#sec7-8}

ScribBERT, dans sa version actuelle, présente au moins ces limites structurantes pour un passage en production :

1. Pas d'hybridation à la récupération : ce qui limite la robustesse sur les requêtes contenant des correspondances exactes.
2. Pas de [*reranking*](#gloss-reranking) : la précision du top-$k$ pourrait être améliorée via un [*cross-encoder*](#gloss-cross-encoder).
...
Ces axes constituent des priorités cohérentes pour la trajectoire de production et seront discutés en perspective au [Ch. 11](#ch11).

```{=latex}
\newpage
```

## Résultats quantitatifs {#ch8}

### Protocole expérimental instancié {#sec8-1}

#### Configurations testées {#sec8-1-1}

Les configurations comparées dans la phase exploratoire correspondent à un plan factoriel 16 modèles de vectorisation × 9 stratégies de [*chunking*](#gloss-chunking) × 6 variantes de récupération = 864 combinaisons, exécuté de bout en bout. Les axes de ce plan sont :

- Axe 1 - modèle de vectorisation : 16 modèles couvrant les familles propriétaire, multilingue [*open-source*](#gloss-open-source), généraliste [*open-source*](#gloss-open-source) et francophone spécialisée (liste détaillée au [§ 7.5.2](#sec7-5-2)).
- Axe 2 - Stratégie de [*chunking*](#gloss-chunking) : 9 stratégies : `fixed-256-0`, `fixed-512-64`, `fixed-1024-128`, `recursive-512-64`, `recursive-1024-128`, `regex-paragraph`, `markdown-1200-50`, `markdown-reference-1000-100`, `semantic-mpnet`.
- Axe 3 - Variante de récupération : 6 combinaisons :
    - `dense-k5` : top-$k$ = 5, sans seuil ;
    - `dense-k10` : top-$k$ = 10, sans seuil ;
    - `dense-k5-thresh` : top-$k$ = 5, seuil de similarité cosinus minimal 0,83 (équivalent à un seuil de distance cosinus maximal 0,17 tel que renvoyé par ChromaDB) ;
    - `dense-k5-neigh` : top-$k$ = 5, contextualisation par voisins $n{-}1$/$n{+}1$ à la récupération ;
    - `hybrid-k5` : fusion dense + BM25 via [*Reciprocal Rank Fusion*](#gloss-reciprocal-rank-fusion), top-$k$ = 5 ;
    - `dense-k20-rerank5` : récupération top-20 puis [*reranking*](#gloss-reranking) [*cross-encoder*](#gloss-cross-encoder) (BGE-reranker-v2-m3), retour top-5.

Le LLM, le [*prompt*](#gloss-prompt) et la température sont gelés à leur valeur de référence ([§ 7.5](#sec7-5)-[7.7](#sec7-7)) pour isoler l'effet des leviers testés. Sur les 864 cellules du plan, 750 sont exploitables.

#### Jeu de test {#sec8-1-2}

Le jeu de test utilisé est constitué de 50 questions annotées manuellement à partir d'une connaissance directe du corpus et des cas d'usage observés (un échantillon est reproduit en [Annexe A](#annexe-A)). La répartition est la suivante :
...
Pour chaque question sont annotés : une réponse de référence rédigée à partir des référentiels, la liste des documents de référence (`relevant_doc_ids`), des paraphrases validées (utilisées pour le protocole de stabilité du [Ch. 6](#ch6)) et des notes contextuelles. Cette taille (50) reste inférieure aux 150-300 questions recommandées : les écarts inter-configurations doivent être lus comme des tendances, et non comme des comparaisons statistiquement décisives. Le passage à un jeu de questions plus large sera abordé ultérieurement.

#### Conditions d'exécution {#sec8-1-3}

- Index vectoriel reconstruit pour chaque modèle de vectorisation testé (réutilisation impossible).
- Journaux complets conservés pour chaque exécution conformément au schéma du [Ch. 5.4.3](#sec5-4-3).

### Résultats récupération {#sec8-2}
...
![Fig. 8.1. Distribution du MRR sur les 750 configurations exploitables du [*benchmark*](#gloss-benchmark) récupération (histogramme et densité). Trait rouge : médiane (0,58) ; bande rouge clair : intervalle interquartile [0,51 ; 0,63]. Les configurations extrémales (meilleure et pire) sont annotées avec leur triplet [*chunking*](#gloss-chunking) / vectorisation / récupération. Source : résultats du [*benchmark*](#gloss-benchmark) récupération (750 cellules exploitables sur les 864 du plan factoriel).](figures/fig_8_1_distribution_mrr.png){#fig:dist-mrr width=90%}

Effet du modèle de vectorisation (MRR moyenne sur l'ensemble des combinaisons [*chunking*](#gloss-chunking) × récupération) :
...
Table: Performance moyenne des 16 modèles de vectorisation sur le [*benchmark*](#gloss-benchmark) récupération (MRR moyen, Hit@5, sur 750 cellules exploitables).

Trois observations méritent d'être soulignées. D'abord, les huit meilleurs modèles s'inscrivent dans une bande de plus ou moins 0,03 de MRR, dans laquelle figurent à la fois des propriétaires (`ada-002`, `embed-3-large`), des multilingues [*open-source*](#gloss-open-source) récents (`nomic-v2`, `qwen3-embed-8b`, `e5-base-ml`, `jina-v3`) et un francophone (`solon-large`). Sur ce corpus, aucun modèle n'écrase les autres, ce qui justifie un arbitrage par les critères pratiques (latence, coût, souveraineté) et non par la seule MRR. Ensuite, les modèles "généralistes anglais" décrochent nettement : `minilm-l6`, `mpnet-base` et `jina-v2-base-en` perdent environ 0,15 de MRR par rapport au peloton de tête, ce qui confirme la nécessité d'un encodeur multilingue sur ce corpus mixte FR/EN ([Ch. 4.1.3](#sec4-1-3)). Enfin, `embed-3-large` n'apporte rien de mesurable par rapport à `ada-002` : les deux modèles donnent des scores rigoureusement identiques sur la plupart des métriques (différence < 0,001 sur l'ensemble du [*benchmark*](#gloss-benchmark)), pour un coût et une latence supérieurs côté `embed-3-large` (~3 300 ms vs ~80 ms par appel de vectorisation, cf. [§ 8.6](#sec8-6)) : l'extra-dimension de `embed-3-large` n'améliore pas la séparation des passages dans ce corpus.

Effet de la stratégie de [*chunking*](#gloss-chunking) (MRR moyenne sur l'ensemble des combinaisons vectorisation × récupération) :

| [*Chunking*](#gloss-chunking) | MRR | Lecture |
|-----------------------------|-----|------------------------------------------------------------|
| `recursive-1024-128` | 0,603 | [*chunks*](#gloss-chunk) larges respectant la structure. Meilleurs résultats |
| `fixed-1024-128` | 0,597 | [*chunks*](#gloss-chunk) larges "naïfs", chevauchement élevé |
| `recursive-512-64` | 0,586 | bon compromis taille/structure |
| `regex-paragraph` | 0,583 | granularité par paragraphe |
| `fixed-512-64` | 0,576 | [*chunks*](#gloss-chunk) moyens avec un chevauchement faible |
| `fixed-256-0` | 0,558 | [*chunks*](#gloss-chunk) courts sans chevauchement |
| `markdown-1200-50` | 0,543 | [*chunks*](#gloss-chunk) markdown larges, chevauchement faible |
| `markdown-reference-1000-100` | 0,541 | [*chunker*](#gloss-chunker) markdown historique ScribBERT (split/merge du parser PDF) |
| `semantic-mpnet` | 0,539 | [*chunking*](#gloss-chunking) sémantique |

Table: Performance moyenne par stratégie de [*chunking*](#gloss-chunking) (MRR sur 750 cellules exploitables).

La hiérarchie confirme une intuition formulée au [Ch. 4.2.2](#sec4-2-2) : sur un corpus normatif, les [*chunks*](#gloss-chunk) larges (1024 [*tokens*](#gloss-token)) battent les [*chunks*](#gloss-chunk) courts, parce qu'ils préservent les blocs "condition + règle + exception" qui constituent l'unité de sens utile. Le [*chunking*](#gloss-chunking) sémantique, plus coûteux à l'ingestion, n'apporte pas de gain mesurable ici. La figure 8.2 croise les deux axes (modèle × [*chunking*](#gloss-chunking)) et fait apparaître plusieurs « îlots » de performance, en particulier le quadrant supérieur gauche qui associe les modèles à plus forte capacité représentationnelle (`nomic-v2`, `ada-002`, `embed-3-large`, `qwen3-embed-8b`) aux [*chunkings*](#gloss-chunking) récursifs et fixes de 1024 [*tokens*](#gloss-token).
...
Effet du mode de récupération (MRR moyenne sur l'ensemble des combinaisons vectorisation × [*chunking*](#gloss-chunking)) :

| Récupération | MRR | Lecture |
|-------------------|-----|----------------------------------------|
| `dense-k20-rerank5` | 0,612 | [*reranking*](#gloss-reranking) : meilleur compromis qualité |
| `hybrid-k5` | 0,596 | hybride dense + BM25 |
| `dense-k10` | 0,576 | top-$k$ large sans [*reranking*](#gloss-reranking) |
| `dense-k5` | 0,565 | version de base |
...
- Le [*reranking*](#gloss-reranking) [*cross-encoder*](#gloss-cross-encoder) (`dense-k20-rerank5`) apporte un gain de +0,047 de MRR par rapport à `dense-k5` (environ +8% relatif), au prix d'une latence supplémentaire (mesurée séparément en [§ 8.6](#sec8-6)). C'est la confirmation expérimentale, sur notre corpus, de la valeur du [*reranking*](#gloss-reranking) évoquée au [Ch. 4.3.3](#sec4-3-3), et un argument fort pour son intégration en production.
- L'hybride dense + BM25 confirme également sa valeur (+0,031 par rapport à `dense-k5`), particulièrement utile pour les requêtes citant explicitement un identifiant de procédure ([Ch. 4.3.2](#sec4-3-2)). Le meilleur top-3 absolu du [*benchmark*](#gloss-benchmark) associe d'ailleurs `qwen3-embed-8b` à `hybrid-k5` sur des [*chunks*](#gloss-chunk) 512-1024 [*tokens*](#gloss-token).
- À l'inverse, la variante `dense-k5-neigh` (ajout systématique des voisins $n{-}1$/$n{+}1$) dégrade la MRR. L'explication est cohérente avec la discussion du [Ch. 4.2.2](#sec4-2-2) : sur des [*chunks*](#gloss-chunk) déjà larges (≥ 512 [*tokens*](#gloss-token)), l'ajout des voisins "dilue" la pertinence du top-5 sans apporter d'information utile, et bruite l'évaluation du "premier passage pertinent". Cette variante reste cependant pertinente quand l'objectif est la génération plutôt que la récupération pure ([§ 8.3](#sec8-3)), où le voisinage restaure des références anaphoriques.
...
Table: Top-5 des configurations récupération combinées ([*chunking*](#gloss-chunking) × *vectorisation* × récupération). Les lignes notant `ada-002 / embed-3-large` correspondent à deux runs distincts dont les métriques coïncident à moins de 0,001 sur l'ensemble des indicateurs reportés : sur ce corpus, l'augmentation de dimension de `embed-3-large` n'apporte aucun gain mesurable par rapport à `ada-002`.

Les meilleures configurations "propriétaires light" (`ada-002` + [*chunks*](#gloss-chunk) 1024) et "[*open-source*](#gloss-open-source) plus + hybride" (`qwen3-embed-8b` + `hybrid-k5`) sont à moins de 1% d'écart en MRR. C'est cette quasi-équivalence, et la simplicité d'exploitation qui motive le choix opérationnel décrit au [§ 7.5.2](#sec7-5-2).

#### Interactions entre *chunking* et modèle de vectorisation {#sec8-2-1}

Les moyennes marginales présentées ci-dessus dissimulent un effet d'interaction qu'il est utile de rendre explicite : le [*chunking*](#gloss-chunking) optimal n'est pas le même pour tous les modèles. Le tableau ci-dessous reporte, pour chaque modèle de vectorisation, le [*chunking*](#gloss-chunking) qui maximise la MRR moyenne (toutes variantes de récupération confondues).

| *Vectorisation* | Meilleur [*chunking*](#gloss-chunking) | MRR | 2ᵉ meilleur [*chunking*](#gloss-chunking) | MRR |
|-----------------------------|------------------------------|-----|------------------------------|-----|
...
Table: Meilleur [*chunking*](#gloss-chunking) par modèle de vectorisation (MRR moyenne, toutes variantes de récupération confondues, 750 cellules exploitables).

Trois régularités se dégagent. Premièrement, les modèles à forte capacité représentationnelle et fenêtre de contexte généreuse (≥ 512 [*tokens*](#gloss-token) (`ada-002`, `embed-3-large`, `nomic-v2`, `qwen3-embed-8b`, `e5-base-ml`, `jina-v3`, `bilingual-fr-en`)) privilégient des [*chunks*](#gloss-chunk) larges (1024 [*tokens*](#gloss-token), parfois 512). Le pattern décrit en moyenne au [§ 8.2](#sec8-2) ("les [*chunks*](#gloss-chunk) larges battent les [*chunks*](#gloss-chunk) courts") tient donc principalement grâce à cette famille. Deuxièmement, les modèles francophones ou multilingues plus spécialisés (`solon-large`, `granite-311m-ml`, `camembert-large`) ainsi que `bge-m3` et `mpnet-base` préfèrent le découpage `regex-paragraph`, qui produit des unités plus courtes et structurées : leurs représentations semblent mieux discriminer entre des paragraphes auto-suffisants qu'entre des sections longues mêlant plusieurs idées. Troisièmement, les modèles compacts à fenêtre courte (`minilm-l6`, max_seq_length = 256 [*tokens*](#gloss-token) ; `e5-small-ml`) sont pénalisés par les [*chunks*](#gloss-chunk) longs qui sont tronqués à l'indexation : leur optimum se déplace mécaniquement vers les configurations 256-512 [*tokens*](#gloss-token).

Conséquence opérationnelle : recommander un [*chunking*](#gloss-chunking) "universel" est trompeur. Le choix doit être fait conjointement avec celui du modèle de vectorisation, et c'est précisément pour cette raison que le plan factoriel complet a été conservé en phase exploratoire plutôt que de figer une stratégie de [*chunking*](#gloss-chunking) a priori. Pour ScribBERT, le couple `recursive-512-64` + `ada-002` retenu en configuration de référence se situe à moins de 0,01 de MRR de l'optimum local de `ada-002` (0,680 sur `fixed-1024-128`), ce qui justifie de privilégier la *configuration récupération* la plus performante (`hybrid-k5`) plutôt que de pousser sur la taille de [*chunk*](#gloss-chunk) (cf. [§ 8.3](#sec8-3) sur l'effet de `hybrid-k5` en génération).

### Résultats génération {#sec8-3}
...
Table: Résultats RAGAS sur les cinq configurations de génération évaluées (50 questions chacune). La dernière ligne, en italique et marquée d'un †, voit ses temps de latence dégradés par la saturation des quotas Azure OpenAI lors de l'exécution en rafale du [*benchmark*](#gloss-benchmark) (erreurs *too many requests*, cf. [§ 8.6](#sec8-6)) ; les scores RAGAS de cette ligne restent en revanche valides puisqu'ils ne dépendent pas de la latence. Une réexécution hors saturation est listée parmi les actions de la trajectoire production ([Ch. 11.2](#sec11-2), Limites du protocole appliqué).
...
![Fig. 8.4. Profil RAGAS des cinq configurations de génération évaluées, sur les quatre dimensions *Faithfulness*, *Answer Relevancy*, *Context Precision* et *Context Recall* (échelle 0 – 1). Chaque polygone correspond à une configuration libellée par  PH15 . Source : résultats du benchmark generation (50 questions par configuration).](figures/fig_8_4_radar_ragas_5_configs.png){#fig:radar-ragas width=75%}

Quatre lectures se dégagent. D'abord, la configuration `hybrid-k5` domine sur les quatre scores RAGAS : elle obtient simultanément la meilleure [*faithfulness*](#gloss-faithfulness) (0,765), la meilleure [*answer relevancy*](#gloss-answer-relevance) (0,756), la meilleure [*context precision*](#gloss-context-precision) (0,687) et le meilleur [*context recall*](#gloss-context-recall) (0,629), tout en étant la plus rapide côté génération (5,09 s médian). C'est la confirmation, côté génération cette fois, du gain d'hybridation déjà observé côté récupération ([§ 8.2](#sec8-2)) : une récupération plus précise se traduit par une génération à la fois plus fidèle aux sources et mieux ciblée sur la question.

Deuxième lecture, plus contre-intuitive : la variante `dense-k5-neigh` améliore la fidélité par rapport à `dense-k5-thresh` (+0,024 de [*faithfulness*](#gloss-faithfulness), +0,020 d'[*answer relevancy*](#gloss-answer-relevance)) malgré sa dégradation observée en récupération pure ([*benchmarks*](#gloss-benchmark) précédents, [§ 8.2](#sec8-2)). Le voisinage $n{-}1$/$n{+}1$, qui ajoute du contexte amont/aval, dégrade la "propreté" du top-5 (donc la MRR) mais aide effectivement le LLM à reconstituer les références anaphoriques et les conditions associées à une règle, exactement le compromis annoncé au [§ 7.4](#sec7-4). C'est une illustration concrète du découplage récupération pure ≠ utilité pour la génération discuté au [Ch. 5.5](#sec5-5).

Troisième point, côté LLM : `gpt-3.5-turbo` plafonne à ≈ 0,75 de [*faithfulness*](#gloss-faithfulness) sur ce corpus. Aucune des trois configurations Azure ne dépasse 0,77, et la dispersion entre configurations RAGAS reste limitée (environ 5 points). Atteindre 0,90 (cible usuelle des [*frameworks*](#gloss-framework) RAG) nécessiterait probablement un modèle de génération plus récent (`gpt-4o`, Claude, Mistral Large), un [*prompt*](#gloss-prompt) plus strict sur la citation, ou un [*reranking*](#gloss-reranking) systématique avant injection.

Enfin, côté alternative locale : Mistral-7B local n'est pas viable en production interactive et reste nettement en retrait sur les scores RAGAS. Avec 36-38 s par question sur GPU, le LLM Mistral-7B est environ 7 fois plus lent que `gpt-3.5-turbo` en appel Azure ; la latence de récupération restant négligeable dans les deux cas (50-130 ms), la chaîne de traitement [*end-to-end*](#gloss-end-to-end) subit le même rapport (environ 36 s contre environ 5 s, cf. [§ 8.6](#sec8-6)). Sa [*faithfulness*](#gloss-faithfulness) plafonne par ailleurs à 0,68 dans la meilleure des deux configurations locales (`recursive-512-64` + `e5-base-ml`), contre 0,76 pour la meilleure chaîne Azure, soit environ 8 points d'écart. Cet écart se creuse encore sur les questions où l'information attendue n'est pas réellement présente dans le corpus : sur Q017 (« Dans quels cas un plan de prévention sous-traitant est-il obligatoire ? »), une vérification manuelle confirme que le seuil réglementaire (400 h/an) ne figure pas dans le doc cité par le test set ; Azure produit alors le refus attendu (« Cette information ne figure pas dans les référentiels consultés ») alors que les deux *chaînes de traitement* locales fabriquent une réponse plausible mais non sourcée. L'observation se retrouve également sur le comportement de citation : `gpt-3.5-turbo` cite systématiquement ses sources en tête de réponse (format `[1][2]` groupé), Mistral-7B + `e5-base-ml` cite relativement correctement (`[1, page 2]`), mais Mistral-7B + `minilm-l6` omet les citations sur une part non négligeable des réponses ou les place de façon incohérente, ce qui explique en partie sa [*context precision*](#gloss-context-precision) plus faible (0,361). L'écart entre les deux runs locaux (0,681 vs 0,612 de [*faithfulness*](#gloss-faithfulness)) confirme par ailleurs que le choix du [*chunker*](#gloss-chunker) et du modèle de vectorisation pèse davantage que le LLM sur le score final : `fixed-256-0` + `minilm-l6` cumule [*chunks*](#gloss-chunk) trop courts et vectorisations trop légères, là où `recursive-512-64` + `e5-base-ml` s'approche un peu plus des standards Azure sans pour autant les rejoindre. Cette voie reste pertinente comme option "souveraineté forte" pour des déploiements sans connectivité Azure. Pour devenir exploitable, elle demanderait de réduire significativement la latence de génération, d'adopter un modèle de vectorisation multilingue plus capacitaire et de renforcer la consigne de citation dans le [*prompt*](#gloss-prompt) système.

L'évaluation des dimensions non automatisables (préservation des modalités santé-sécurité, sûreté opérationnelle, complétude experte, Ch. 5.1.2 et 5.2.2) a été menée manuellement sur un sous-échantillon stratifié de 15 questions critiques (issues majoritairement des catégories conditionnelle et procédurale, criticité élevée), conformément au protocole hybride du Ch. 5.2.3. Sur la configuration de référence (`recursive-512-64` + `ada-002` + `hybrid-k5` + `gpt-3.5-turbo`), les 15 réponses préservent correctement les « doit », « peut », « ne doit pas » : aucune inversion de modalité n'a été observée sur ce sous-échantillon, ce qui suggère que la consigne *prompt* de reprise textuelle des verbes normatifs joue effectivement son rôle. Le constat reste néanmoins à confirmer sur un échantillon plus large, le risque demeurant théorique quand le top-5 est dominé par des reformulations pédagogiques plutôt que par le référentiel. Côté sûreté opérationnelle, aucune réponse n'a produit d'instruction dangereuse ou contraire aux référentiels, y compris sur les questions adversariales (cf. § 9.4.3). En revanche, la complétude experte est plus inégale : sur 6 questions conditionnelles incluses dans le sous-échantillon, 2 omettent au moins une exception ou un cas particulier pourtant présent dans le document source, ce qui rejoint la catégorie « omission d'exception » identifiée en § 9.2. Cette évaluation reste à étendre à un échantillon plus large (objectif 50 questions, idéalement avec deux annotateurs pour mesurer l'accord inter-juges) avant d'être consolidée en métrique de référence.

### Résultats stabilité {#sec8-4}

Le protocole du Ch. 6.5 a été appliqué à la **configuration de stabilité** (`markdown-1200-50` + `ada-002` + `dense-k5-thresh` + `azure-gpt35`), choisie comme représentative du POC actuellement déployé (cf. clarification en tête de Partie III : cette chaîne reprend le couple *chunker* / *vectorisation* du POC, avec un top-$k$ réduit à 5 pour rester alignée sur les variantes effectivement testées dans le plan factoriel du § 8.1.1). Pour chacune des 50 questions, $n=10$ exécutions ont été lancées à seed et paramètres constants (sources de variance : non-déterminisme du LLM, ordre des passages à score égal à la sortie de ChromaDB) ; en parallèle, les paraphrases annotées dans le jeu de test ont été soumises pour mesurer la consistance sémantique de la réponse. Résultats (n=50 questions) :
...
Table: Indicateurs de stabilité de la configuration de stabilité ScribBERT (50 questions, 10 *runs* par question).
...
![Fig. 8.5. Distribution des quatre indicateurs de stabilité sur les 50 questions du jeu de test (configuration de stabilité `markdown-1200-50 / ada-002 / dense-k5-thresh / azure-gpt35`). De gauche à droite : *Stability@retrieval* (Jaccard inter-runs sur le top-5 récupéré), *Stability@citations* (Jaccard sur les sources effectivement citées dans la réponse), *Stability@answer* (BERTScore F1 moyen entre paires de runs) et *Robustness@paraphrases* (BERTScore F1 entre la réponse originale et celle obtenue sur une paraphrase). Mesures à seed et paramètres constants, $n = 10$ runs par question. Médiane annotée au-dessus de chaque boîte. Source : résultats de la campagne de stabilité.](figures/fig_8_5_boxplot_stabilite.png){#fig:box-stability width=90%}

![Fig. 8.6. Stabilité inter-runs (axe X,  PH5 ) vs robustesse aux paraphrases (axe Y,  PH6 ), une question par point. La diagonale x = y matérialise le cas idéal où une reformulation a le même effet qu'une nouvelle exécution stochastique ; tout point sous la diagonale correspond à une question plus sensible aux paraphrases qu'à la variance intra-run. Les six questions présentant le plus grand écart sont étiquetées par leur identifiant. Source : résultats de la campagne de stabilité (mêmes données qu'en Fig. 8.5).](figures/fig_8_6_scatter_interruns_vs_paraphrases.png){#fig:scatter-stability width=85%}

\needspace{30\baselineskip}

Quatre constats. Premier point, la récupération est parfaitement reproductible (Jaccard 1,000 sur les 50 questions, 0 écart-type) : la couche vectorielle ChromaDB ne contribue à aucune variabilité observable dans cette configuration. Toute variation de réponse provient donc de la couche génération. Deuxièmement, la génération est presque déterministe à requête fixe (BERTScore F1 inter-runs ≈ 0,94, écart-type 0,024), résultat cohérent avec la température 0,05 imposée au [§ 7.7](#sec7-7). Le taux de basculement sur le choix des sources citées reste limité mais non nul (Stability@citations = 0,935, soit 6,5% de variation moyenne) : la cible "0,95+" recommandée pour un déploiement critique au [Ch. 6.3](#sec6-3) est presque atteinte, mais pas encore validée. Troisième constat, plus préoccupant : la robustesse aux paraphrases est nettement plus faible (0,77 vs 0,94, soit 17 points d'écart). Reformuler la même question en français courant fait varier sensiblement la réponse produite, ce qui ne veut pas dire que la réponse est fausse, mais qu'elle n'est pas invariante. Pour un assistant santé-sécurité où l'utilisateur peut formuler la même intention de plusieurs façons, c'est l'indicateur prioritaire à améliorer, par exemple via une étape de normalisation de requête en amont de la récupération ([Ch. 4.3.6](#sec4-3-6)). Enfin, ces tests ne couvrent qu'une configuration sur les 750 testées en récupération. Une mesure de stabilité comparative entre les meilleures configurations (notamment `hybrid-k5` et `dense-k20-rerank5`) reste à réaliser pour vérifier que les gains de fidélité observés au [§ 8.3](#sec8-3) ne se font pas au prix d'une variance inter-runs accrue.

### Résultats *end-to-end* et couplage entre récupération et génération {#sec8-5}

En croisant les résultats des [§ 8.2](#sec8-2) et [§ 8.3](#sec8-3), plusieurs enseignements ressortent sur le couplage récupération-génération.

Premièrement, la manière dont sont ordonnés les résultats de la récupération n'est pas préservée à la génération. Sur les trois configurations Azure évaluées, la meilleure côté récupération n'est pas la meilleure côté [*faithfulness*](#gloss-faithfulness) : `hybrid-k5` (MRR 0,596) domine les quatre scores RAGAS là où `dense-k5-thresh` (MRR 0,564) plafonne. L'écart d'environ 5 % de MRR se traduit par environ 4 % de [*faithfulness*](#gloss-faithfulness) (Ch 5.5) : un meilleur rappel améliore la fidélité, mais l'effet est amorti par la couche LLM, qui sauve parfois un top-5 imparfait et manque parfois un top-5 correct.

Deuxièmement, la [*context precision*](#gloss-context-precision) RAGAS est un meilleur signal de fidélité que le Recall@k. La configuration `dense-k5-neigh` obtient un [*context recall*](#gloss-context-recall) correct (0,592) mais une [*context precision*](#gloss-context-precision) faible (0,418), précisément parce qu'elle injecte deux fois plus de [*tokens*](#gloss-token) par [*chunk*](#gloss-chunk) via le voisinage : le modèle dispose de la bonne information mais aussi de plus de bruit, ce qui dégrade légèrement les autres scores RAGAS. C'est l'illustration directe du compromis "plus de contexte = plus de bruit" et de l'effet [*lost in the middle*](#gloss-lost-in-the-middle) discutés au [Ch. 4](#ch4).

Enfin, la configuration `dense-k5-thresh` (seuil de distance maximal fixé à 0,17) joue effectivement son rôle de garde-fou : 10 questions in-scope sur 46 (les 4 questions hors-périmètre reçoivent par ailleurs le refus attendu) reçoivent une réponse de type « information non trouvée dans les référentiels », contre 5 sur la configuration `hybrid-k5` de référence. Parmi ces 10 refus, 6 sont à tort (Q005, Q009, Q013, Q026, Q043 et Q044, dont le contexte récupéré contient au moins partiellement la réponse) et 4 sont légitimes. Côté `hybrid-k5`, les 5 refus sont tous légitimes (cf. § 9.2). Le filtre joue donc son rôle, mais sa sévérité coûte 6 refus à tort sur le sous-ensemble in-scope. Une calibration plus fine de ce seuil (entre 0,10 et 0,25) pourrait être une amélioration à fort levier dans la trajectoire production (Ch. 11).

### Coût opérationnel {#sec8-6}

Les latences mesurées sur l'ensemble du [*benchmark*](#gloss-benchmark) se décomposent comme suit. La colonne `latency_s` du [*benchmark*](#gloss-benchmark) récupération mesure uniquement l'appel de vectorisation de la requête (et l'aller-retour web pour les modèles via API Azure), sans inclure la recherche ChromaDB elle-même (négligeable sur un index de cette taille). Les temps de génération discutés au [§ 8.3](#sec8-3) (colonnes `t_retrieval_s` et `t_generation_s` du [*benchmark*](#gloss-benchmark) génération) sont mesurés séparément et reportés ici dans le tableau du bas pour donner une vision [*end-to-end*](#gloss-end-to-end).
...
La figure 8.7 croise ces latences avec la MRR moyenne pour visualiser le compromis qualité/coût : `ada-002` se situe juste sur le front de Pareto, dominé en latence brute par les modèles [*open-source*](#gloss-open-source) légers (qui ne gagnent rien en qualité) et nettement préférable à `embed-3-large` qui n'apporte aucun gain mesurable de MRR pour une latence de l'ordre de 40 fois supérieure.
...
`ada-002` est donc nettement plus lent en latence brute de vectorisation que la majorité des modèles [*open-source*](#gloss-open-source) légers exécutés en local sur GPU (le surcoût vient entre autres de l'aller-retour réseau vers l'[*endpoint*](#gloss-endpoint) Azure), mais reste très en-dessous des autres modèles propriétaires (`embed-3-large`) et des très gros [*open-source*](#gloss-open-source) (`qwen3-embed-8b`). Son choix opérationnel ne se justifie donc pas par la latence de vectorisation, négligeable par rapport à la génération LLM, mais plutôt par sa qualité de récupération et l'absence d'hébergement GPU côté Bouygues.
...
Table: Décomposition de la latence [*end-to-end*](#gloss-end-to-end) de la chaîne de traitement RAG ScribBERT (configuration de référence Azure).

La figure 8.8 reporte cette décomposition pour les cinq configurations de génération évaluées : la couche LLM domine systématiquement la latence (entre 88% et 99% du temps total selon la chaîne de traitement), tandis que la récupération reste résiduelle, y compris pour les configurations qui s'appuient sur un [*embedding*](#gloss-embedding) API ou un voisinage de [*chunks*](#gloss-chunk).
...
À titre de comparaison, la chaîne de traitement locale Mistral-7B atteint 36 à 38 s par question (essentiellement décodage GPU), hors cible pour une expérience interactive. La configuration Azure dégradée par les erreurs "too many requests" monte à 55 s médian, mais ce chiffre est lié au [*benchmark*](#gloss-benchmark), qui enchaîne les 50 questions en rafale et sature les quotas des APIs. Un utilisateur réel, espaçant naturellement ses requêtes, ne déclencherait pas ce comportement et resterait sur des temps comparables à la configuration de référence.

Coût par requête (estimation indicative au tarif Azure OpenAI public) :

- Vectorisation requête (`ada-002`, environ 50 [*tokens*](#gloss-token)) : environ 0,000005 €
- Génération `gpt-3.5-turbo` (pour 2 500 [*tokens*](#gloss-token) contexte + 300 [*tokens*](#gloss-token) réponse) : environ 0,002 €
- Total environ 0,002 €/requête, soit moins de 0,20 € pour 100 questions.

Estimation du coût total de la campagne de [*benchmark*](#gloss-benchmark) (estimation indicative, hors temps GPU local) :

- Construction des index vectoriels : seuls les modèles de vectorisation propriétaires (`ada-002`, `embed-3-large`) ont engendré un coût API. Avec 9 stratégies de [*chunking*](#gloss-chunking) x 2 modèles de vectorisation et un corpus d'environ 1,3 million de [*tokens*](#gloss-token) par stratégie, cela représente environ 23 millions de [*tokens*](#gloss-token) embeddés cumulés, soit environ 1,2 € pour `ada-002` (0,0001 €/1k [*tokens*](#gloss-token)) et environ 1,5 € pour `embed-3-large` (0,00013 €/1k [*tokens*](#gloss-token)). Les autres modèles de vectorisation étant exécutés en local sur GPU, leur coût se limite à la consommation électrique.
...
- Génération `gpt-3.5-turbo` sur les campagnes génération (3 configs Azure x 50 questions) et stabilité (1 config x 50 questions x 10 runs) : environ 750 appels avec environ 3 000 [*tokens*](#gloss-token) de contexte et 300 [*tokens*](#gloss-token) de réponse, soit environ 1,5 €.
- Évaluation RAGAS (4 métriques × 5 configs × 50 questions, chaque métrique faisant plusieurs appels [*LLM-juge*](#gloss-llm-as-judge) avec un contexte de 2 à 3 k [*tokens*](#gloss-token)) : c'est le poste le plus important, environ 5 à 7 € selon le détail des [*prompts*](#gloss-prompt) internes RAGAS.
- Total estimé : environ 10 à 15 € pour l'ensemble de la phase exploratoire Azure, hors coût matériel/électrique du GPU local utilisé pour les modèles de vectorisation [*open-source*](#gloss-open-source) et pour les deux runs Mistral-7B.
...
L'ajout d'un [*reranking*](#gloss-reranking) [*cross-encoder*](#gloss-cross-encoder) (`bge-reranker-v2-m3`) en local représenterait un surcoût matériel plus qu'un surcoût monétaire, et ajouterait de l'ordre de 0,3 à 0,5 s par requête (temps nécessaire au [*reranker*](#gloss-reranker) pour re-scorer les 20 [*chunks*](#gloss-chunk) issus de la récupération initiale et n'en conserver que les 5 meilleurs), d'après les essais préliminaires inclus dans `dense-k20-rerank5`.

```{=latex}
\newpage
```

## Analyse qualitative et étude d'erreurs {#ch9}

### Méthodologie {#sec9-1}

La phase de test utilisateur menée pendant le projet a recueilli des retours majoritairement positifs. Ce chapitre instancie la typologie d'erreurs en 8 catégories définie au [Ch. 5.5.4](#sec5-5-4) (échec de récupération, bruit de récupération, hallucination factuelle, omission d'exception, inversion de modalité, contradiction silencieuse, refus à tort, hors-périmètre accepté) sur les sorties de la configuration de référence (`recursive-512-64` + `ada-002` + `hybrid-k5` + `gpt-3.5-turbo`) sur les 50 questions du jeu de test.
...
### Typologie d'erreurs observées {#sec9-2}

Pour chiffrer chaque catégorie d'erreur sur les 50 questions, les scores RAGAS calculés question par question ([*context recall*](#gloss-context-recall), [*context precision*](#gloss-context-precision), [*faithfulness*](#gloss-faithfulness)) sont utilisés comme indicateurs : un seuil simple sur ces scores permet de classer chaque question dans la bonne catégorie, et un script compte ensuite combien de questions tombent dans chacune.
...
|--------------------------------|------------------------------------|----------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| Échec de récupération | 10/50 (20%) | Q003 « Que faire si l'O2 d'un espace confiné est < 19,5% ? » le [*chunk*](#gloss-chunk) de `REF-2223` citant le seuil sort du top-5, remplacé par des [*chunks*](#gloss-chunk) plus généraux sur la ventilation | Le [*chunk*](#gloss-chunk) normatif (court, dense) est mal classé face aux [*chunks*](#gloss-chunk) longs qui répètent le concept large « espace confiné ». Ajouter à l'index une paraphrase titre + résumé pour chaque référentiel, et tester `dense-k20-rerank5` |
| Bruit de récupération | 9/50 (18%) | Q005 « Différence entre permis de feu et permis d'intervention ATEX ? » top-5 dominé par le « permis de feu », un seul [*chunk*](#gloss-chunk) ATEX | Sur les questions comparatives, l'entité la plus représentée noie la seconde. Décomposer en sous-requêtes ([Ch. 4.3.6](#sec4-3-6)) ou appliquer un [*cross-encoder*](#gloss-cross-encoder) sur chaque moitié du contexte |
...
| Catégorie d'erreur | Fréquence (config réf) | Exemple représentatif | Interprétation |
|---------------------------------|------------------------------------|---------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------|
| Contradiction silencieuse | 0/50 (config référence) ; 1/50 sur Mistral local | Cf. Q046 ci-dessous (Cas 6, [§ 9.3](#sec9-3)) | Sur la config de référence Azure, aucune contradiction silencieuse détectée. Sur les *chaînes* Mistral-7B local, le risque réapparaît (réponse longue plausible sans source) : prévoir un garde-fou applicatif (refus forcé si [*faithfulness*](#gloss-faithfulness) nul et réponse longue) |
...
| Inversion de modalité | 0/15 sur le sous-échantillon manuel | Aucune inversion observée : les modalités normatives (« doit », « ne doit pas », « peut ») sont restituées telles quelles dans les réponses examinées | Résultat encourageant à confirmer sur un échantillon plus large : la consigne *prompt* « reprendre les verbes "doit", "ne doit pas", "peut" tels quels » semble efficace, mais le risque demeure quand le top-5 est dominé par des reformulations pédagogiques plutôt que par le référentiel |

Table: Erreurs liées au [*prompt*](#gloss-prompt) et aux garde-fous (configuration de référence ScribBERT, 50 questions).

La figure 9.1 reporte la fréquence de chaque catégorie obtenue par classification automatique *non-exclusive* (une même question peut tomber dans plusieurs catégories ; les totaux peuvent donc excéder 50). La catégorie *omission d'exception* est sous-représentée par cette classification purement automatique fondée sur les seuils RAGAS, et est à compléter par l'évaluation humaine ciblée mentionnée au § 8.3 (sous-échantillon stratifié de 15 questions). La catégorie *inversion modalité*, non détectable par les seuls scores RAGAS, reste fixée à 0 dans la figure ; cette valeur coïncide avec celle observée à l'évaluation humaine (0/15, cf. § 9.2).
...
Q : « Pourquoi la notion de "ligne de feu" est-elle centrale dans la prévention des accidents ? » F 0,00 / CR 0,00. Le document cible `First Alert Stay Risk Aware in the Line of Fire - ALIGN` est en anglais alors que la question est posée en français, et il s'agit d'un format d'alerte court : l'appariement cross-lingue échoue, le top-5 ne remonte aucun [*chunk*](#gloss-chunk) de ce document. Sur la configuration de référence Azure (`hybrid-k5` + `gpt-3.5-turbo`), le système retombe correctement sur le refus standardisé (« Cette information ne figure pas dans les référentiels consultés. »), grâce à la consigne [*prompt*](#gloss-prompt) qui interdit de produire une réponse non sourcée. Sur les deux *chaînes de traitement* Mistral-7B local en revanche, la même question déclenche une réponse longue, structurée et thématiquement plausible, fabriquée à partir de la connaissance générale du modèle, sans qu'aucun [*chunk*](#gloss-chunk) source ne la valide. C'est l'archétype de la contradiction silencieuse : réponse « d'allure experte » sans ancrage documentaire, situation la plus problématique pour la confiance utilisateur. Sur la configuration de référence, ce risque est neutralisé par le [*prompt*](#gloss-prompt) (cf. [Ch. 7.7](#sec7-7)) ; sur la voie souveraine Mistral local, il faudra un garde-fou applicatif explicite.

### Cas limites et ambiguïtés {#sec9-4}

#### Acronymes et jargon métier {#sec9-4-1}
...
Ce résultat ne signifie pas que l'acronyme « aide » en soi : il s'explique surtout par un facteur de confusion. Les questions sans acronyme sont majoritairement des formulations ouvertes (« pourquoi… », « comment… ») intrinsèquement plus difficiles, alors que les questions avec acronyme sont souvent factuelles et s'ancrent sur un terme à forte spécificité lexicale, présent tel quel dans les [*chunks*](#gloss-chunk) d'origine (titres, listes, tableaux), ce qui suffit à une vectorisation dense généraliste pour les apparier, sans nécessiter d'expansion.

Conséquence pratique, le levier d'expansion d'acronymes (évoqué au [Ch. 4.3.6](#sec4-3-6) parmi les techniques de [*query rewriting*](#gloss-query-rewriting)) devient *a priori* secondaire sur ce corpus. L'effort doit plutôt se porter sur les questions ouvertes en langage naturel, qui constituent le véritable point faible du système.

#### Multilinguisme et alternance codique {#sec9-4-2}

Sur 9 questions en Anglais et 41 questions Françaises, la config de référence donne :
...
Ce résultat est, là encore, contre-intuitif : les questions formulées en Anglais performent *mieux*. Deux explications : 

1. biais d'échantillonnage (n=9 EN, dispersion forte, non significative statistiquement)
2. les questions Anglaises du jeu de test sont majoritairement factuelles ou procédurales claires (« What PPE… », « What permits… », « What is the immediate procedure… »), alors que les FR couvrent plus de questions justificatives ou conditionnelles plus difficiles.

La situation inédite (question Française sur doc cible Anglais) a en revanche été observée sur quelques cas dont Q046 ([§ 9.3](#sec9-3), Cas 6) : le doc `First Alert Stay Risk Aware in the Line of Fire - ALIGN`, court et uniquement en anglais, n'est pas remonté alors que la question est posée en français. `ada-002` est multilingue et le top-5 mélange spontanément les deux languers selon la pertinence, mais l'appariement échoue quand le doc cible anglophone est trop court pour offrir un signal sémantique suffisant face à une requête française plus longue (c'est l'asymétrie de longueur qui creuse l'écart cosinus, pas la différence de langue seule). Le LLM répond toujours dans la langue de la question. Ces observations doivent être confirmées sur un jeu de test équilibré (objectif [§ 11.2.1](#sec11-2-1)).

#### Hors-périmètre {#sec9-4-3}
...
Sur ce périmètre, c'est la consigne *prompt* (« si aucun extrait pertinent, répondre exactement "Cette information ne figure pas dans les référentiels consultés" ») qui joue le rôle de garde-fou sur la configuration de référence `hybrid-k5` : aucun *chunk* du corpus ne traite ces sujets adversariaux, le contexte injecté au LLM est sémantiquement éloigné, et le modèle bascule sur le refus standardisé. Le filtre par distance de `dense-k5-thresh` (seuil 0,17) jouerait le même rôle de façon plus stricte en amont, en empêchant carrément les *chunks* de remonter, mais sa sévérité coûte aussi des refus à tort sur les questions in-scope (§ 8.5). Dans les deux cas, le garde-fou est passif : une question adversariale proche d'un sujet du corpus (ex. « comment ne pas porter d'EPI sans se faire prendre ? ») pourrait remonter des *chunks* plausibles et passer la barrière.

La figure 9.2 récapitule la performance moyenne par type de question. Les factuelles et procédurales atteignent les meilleurs scores RAGAS sur les quatre dimensions ; les conditionnelles décrochent nettement sur le [*context recall*](#gloss-context-recall) (signe d'exceptions ou de fondements omis lors de la récupération) ; les justificatives présentent une [*faithfulness*](#gloss-faithfulness) plus dispersée, cohérent avec la difficulté à fonder une explication sur des [*chunks*](#gloss-chunk) normatifs ; et les hors-périmètre se distinguent par une [*faithfulness*](#gloss-faithfulness) moyenne nulle correspondant aux refus contrôlés correctement déclenchés. Cette stratification motive l'analyse fine des biais conduite au [§ 9.5](#sec9-5).
...
Biais de corpus (observé indirectement). Les questions liées au travail en hauteur, EPI et énergies dangereuses obtiennent les meilleurs scores RAGAS ([*faithfulness*](#gloss-faithfulness) ≥ 0,87 en moyenne), ce qui reflète à la fois la qualité des questions et la densité documentaire sur ces sujets dans le corpus. Les sujets sous-documentés (santé mentale, risque chimique avancé) n'apparaissent quasiment pas dans le jeu de test actuel, et l'évaluation ne dit donc rien de leur traitement par le système. C'est un angle mort à corriger lors de l'extension du jeu de test ([Ch. 11.2.1](#sec11-2-1)).

Biais d'ordre d'index (effet faible). ChromaDB en HNSW n'introduit pas de biais d'ordre dans la récupération (les résultats sont triés par similarité), mais l'ordre de citation dans la réponse générée pourrait refléter l'ordre d'arrivée des [*chunks*](#gloss-chunk) dans le contexte. La campagne de stabilité ([§ 8.4](#sec8-4)) montre $1 - \mathrm{Stab@cit} = 1 - 0{,}935 = 0{,}065$, soit 6,5% de variation moyenne sur l'ensemble des sources citées d'un run à l'autre, cohérent avec cet effet et tolérable en l'état.

Biais de longueur (effet fort, à neutraliser). Les réponses générées par `gpt-3.5-turbo` sur la configuration de référence font en moyenne 177 mots (médiane 170, maximum 481), soit environ 230 [*tokens*](#gloss-token) (jusqu'à plus de 600 pour les réponses les plus développées). La corrélation entre longueur de la réponse et scores RAGAS est très marquée : Pearson $r = +0{,}64$ avec la [*faithfulness*](#gloss-faithfulness) et $r = +0{,}63$ avec l'[*answer relevancy*](#gloss-answer-relevance). Stratifié par longueur de réponse, l'effet est net :
...
La figure 9.3 visualise cette corrélation point par point sur les deux métriques les plus sensibles, [*faithfulness*](#gloss-faithfulness) et [*answer relevancy*](#gloss-answer-relevance), et matérialise la régression linéaire associée.

![Fig. 9.3. Effet de la longueur de la réponse générée (en mots, axe X) sur les scores RAGAS de *Faithfulness* (gauche) et *Answer Relevancy* (droite). Une question = un point ($n = 50$). La droite rouge pointillée correspond à une régression linéaire simple, le coefficient de Pearson est indiqué dans le titre de chaque sous-figure. Source : résultats détaillés par question de la configuration de référence  PH1 , longueur calculée à partir du texte de chaque réponse.](figures/fig_9_3_scatter_longueur_ragas.png){#fig:length-ragas width=95%}

Une partie de cette corrélation est légitime : les réponses très courtes sont essentiellement des refus contrôlés, par construction notés [*faithfulness*](#gloss-faithfulness) = 0 par RAGAS. Mais une partie correspond à un biais documenté du [*LLM-as-judge*](#gloss-llm-as-judge) : les juges LLM tendent à mieux noter les réponses verbeuses, plus structurées et plus enrobées, indépendamment de leur exactitude factuelle [@Zheng2023JudgeBias]. Sur les questions factuelles courtes (« Quels sont les EPI obligatoires ? »), la sur-longueur amplifie en outre le [*context recall*](#gloss-context-recall) mécaniquement, puisque toutes les sources sont de facto citées dans une réponse plus longue. Deux mitigations sont à étudier en parallèle de la prochaine itération : durcir la consigne de concision dans le [*prompt*](#gloss-prompt) système et calibrer les scores RAGAS sur un échantillon annoté humainement ([§ 5.2.3](#sec5-2-3)).

Biais linguistique (effet faible sur la moyenne, mais asymétrie structurelle à investiguer). Les questions FR et EN du jeu de test ont des longueurs comparables (16,6 vs 16,9 mots en moyenne), donc l'asymétrie observée au § 9.4.2 (FR 0,73 vs EN 0,91 de *faithfulness*) ne vient pas d'un effet longueur côté requête. Elle s'explique principalement par la composition du sous-échantillon EN (n=9), majoritairement factuel et procédural, contre un sous-échantillon FR plus chargé en questions justificatives et conditionnelles. Un mécanisme distinct, observé sur Q046 (§ 9.3, Cas 6 et § 9.4.2), apparaît en revanche quand le document cible est lui-même court : le `Safety Alert Line of Fire` tient sur un seul *chunk* de 265 mots, soit nettement moins que la taille moyenne d'un *chunk* `recursive-512-64`. Face à une requête FR de longueur comparable mais sémantiquement plus diffuse, l'écart cosinus se creuse mécaniquement. Le mécanisme à l'œuvre n'est pas une sensibilité de la mesure à la norme (la similarité cosinus configurée dans ChromaDB est par construction invariante à la norme des vecteurs (cf. § 4.3.1)) mais plutôt une dérive de la *direction* elle-même : les encodeurs denses sont entraînés sur des passages dont la longueur typique se situe autour de 256–512 *tokens* et agrègent les représentations par *mean-pooling*. Sur un *chunk* nettement plus court, la moyenne est moins lissée et l'orientation finale est davantage dominée par quelques *tokens* spécialisés (« UXO », « *line of fire* »), ce qui creuse l'angle avec une requête FR sémantiquement plus large et finit par éjecter ce document du top-5. Le levier n'est donc pas linguistique au sens strict, mais structurel : prévoir au moins une paraphrase-résumé indexée pour les documents très courts, ou tester un *chunking* avec padding contextuel pour homogénéiser la longueur des unités indexées. Cette piste sera évaluée lors de l'extension du jeu de test EN (Ch. 11.2.1) avant d'écarter toute hypothèse de biais linguistique latent.
...
### Retours utilisateurs (phase de test) {#sec9-6}
...
1. La prise en charge des questions comparatives ou contrastives (par exemple « Quelles différences entre les procédures de levage BYTP et les exigences réglementaires ? »). Le top-$k$ actuel agrège bien jusqu'à 3-4 documents distincts dans une même réponse, mais il ne sait pas isoler explicitement deux sources concurrentes pour les mettre face à face : il n'existe aujourd'hui aucun mécanisme dans le [*prompt*](#gloss-prompt) classique pour forcer la sélection équilibrée de [*chunks*](#gloss-chunk) issus de référentiels différents puis structurer la réponse en comparaison point à point. C'est précisément ce besoin qui a justifié l'intégration au POC de la fonctionnalité de [*gap analysis*](#gloss-gap-analysis) (cf. [§ 9.3](#sec9-3), Cas 4) : l'utilisateur sélectionne manuellement deux sous-ensembles de documents et la même requête est exécutée sur chacun séparément, ce qui garantit l'isolation des sources et rend les écarts directement lisibles.
2. L'exploitation des tableaux et schémas des PDF (non gérés dans le POC, cf. [§ 7.2.3](#sec7-2-3))
...
### Le cadre réglementaire européen : l'AI Act {#sec10-1}

#### Classification du système {#sec10-1-1}
...
#### Obligations applicables (en hypothèse haut risque) {#sec10-1-2}
...
- Qualité des données d'entraînement : moins applicable ici (pas de [*fine-tuning*](#gloss-fine-tuning)), mais la qualité du corpus est un équivalent fonctionnel.
...
Le protocole d'évaluation proposé dans ce mémoire contribue directement à plusieurs de ces exigences : la mesure de la fiabilité ([Ch. 5](#ch5)-[6](#ch6)), la traçabilité des sources, la documentation des choix techniques ([Ch. 7](#ch7)), constituent des éléments mobilisables pour la conformité.

#### Articulation avec d'autres référentiels {#sec10-1-3}
...
### RGPD et données internes {#sec10-2}
...
2. Confidentialité des documents internes : le choix d'un hébergement local au LabTP ([§ 7.2.2](#sec7-2-2)) garantit la non-exposition à des fournisseurs cloud externes pour le POC. La bascule éventuelle vers de l'hébergement cloud en production exigerait une analyse complémentaire, idéalement avec contrats DPA appropriés.
...
### Responsabilité en contexte santé-sécurité {#sec10-3}

#### La question de la responsabilité {#sec10-3-1}
...
#### L'avertissement comme mesure de mitigation {#sec10-3-2}
...
Cet avertissement est une mesure nécessaire mais non suffisante : la jurisprudence européenne sur les outils d'aide à la décision tend à considérer qu'un avertissement ne dégage pas l'éditeur de toute responsabilité, particulièrement si l'outil est présenté comme "expert" ou "fiable" (l'AI Act impose la transparence sur la nature artificielle et les limites du système, mais ne se limite pas à cela : il exige aussi gestion des risques, documentation, traçabilité, contrôle humain et robustesse mesurée, cf. [§ 10.1.2](#sec10-1-2)). Les renforcements possibles incluent :

- afficher un score de confiance par réponse, pour maintenir la vigilancede de l'utilisateur, comme évoqué au Chapitre 6 lorsque nous avions exploré la stabilité et la répétabilité des systèmes,
...
#### Supervision humaine {#sec10-3-3}

Le principe de [*human-in-the-loop*](#gloss-human-in-the-loop) est central pour les systèmes IA en domaine critique. Pour ScribBERT, cela peut prendre plusieurs formes :
...
### Gouvernance d'un RAG d'entreprise {#sec10-4}

L'industrialisation impose une discipline de gouvernance que le prototype peut tolérer, mais que la production exige :

- Versioning : chaque mise en production identifie sans ambiguïté la version du modèle de vectorisation, du LLM, du corpus, du [*prompt*](#gloss-prompt) et du code applicatif.
...
- [*Audit trail*](#gloss-audit-trail) : chaque réponse produite est journalisée avec l'ensemble des éléments permettant de la rejouer (cf. [Ch. 5.4.3](#sec5-4-3)).
...
### Acceptabilité et conduite du changement {#sec10-5}
...
1. La confiance, gagnée par la qualité des réponses et par la transparence sur les sources. Les retours utilisateurs ([§ 9.6](#sec9-6)) confirment que la présence systématique des citations est un facteur clé d'adoption.
...
## Discussion et perspectives {#ch11}

### Interprétation des résultats et synthèse des enseignements {#sec11-1}

Les Parties I et II ont posé un cadre théorique et méthodologique pour évaluer un RAG dans un contexte critique. La Partie III a montré comment ce cadre s'applique à un cas réel (ScribBERT) : la phase exploratoire a produit un *benchmark* de 750 configurations exploitables de récupération, une campagne génération sur 5 configurations (3 Azure + 2 locales) et une campagne stabilité sur la configuration de stabilité (cf. clarification de vocabulaire en tête de Partie III). Les résultats permettent à la fois d'arbitrer les choix opérationnels du POC (cf. Ch. 7-8) et d'identifier précisément ce qui reste à instrumenter (préservation des modalités, stabilité comparative entre meilleures variantes, validation humaine sur les questions critiques). L'instanciation complète du protocole sur les meilleures variantes et l'extension du jeu de test à 150-300 questions constituent les deux suites naturelles de ce travail.
...
2. Les choix d'ingénierie ([*chunking*](#gloss-chunking), contextualisation, filtrage par score) ont un impact comparable à celui du choix du modèle : il est tentant de centrer l'attention sur le LLM, mais l'expérience ScribBERT confirme qu'un [*chunking*](#gloss-chunking) adapté au corpus et un filtrage de seuil bien calibré pèsent au moins autant.
3. La stabilité est sous-évaluée dans les [*frameworks*](#gloss-framework) usuels : pour un système en production sur un sujet critique, la variance inter-runs et la robustesse aux paraphrases méritent un protocole dédié ([Ch. 6](#ch6)).
...
### Limites méthodologiques {#sec11-2}

#### Limites du jeu de test {#sec11-2-1}

Le jeu de test utilisé pour ce mémoire (50 questions) est inférieur aux 150-300 questions recommandées au [Ch. 5.3.4](#sec5-3-4) pour des comparaisons statistiques fines : les écarts inter-configurations observés au [§ 8.2](#sec8-2) doivent être lus comme des tendances cohérentes, non comme des comparaisons statistiquement décisives. Une priorité immédiate est l'extension à 150-300 questions stratifiées, avec annotation des passages de référence et des réponses de référence par des experts P2S directement, et en augmentant en particulier la part anglophone (9/50 actuellement).

#### Limites du protocole appliqué {#sec11-2-2}

Le [*benchmark*](#gloss-benchmark) a couvert 750 cellules exploitables en récupération et 5 configurations en génération (3 Azure + 2 locales). En revanche, la campagne de stabilité n'a porté que sur une seule configuration. Deux chantiers restent donc à mener pour clore le protocole :
...
#### Limites du périmètre {#sec11-2-3}
...
#### Précautions d'interprétation {#sec11-2-4}
...
### Apports du travail {#sec11-3}

#### Apports théoriques {#sec11-3-1}

- Une définition opératoire de la fiabilité d'un RAG ([Ch. 3.3](#sec3-3)) qui décompose le concept en cinq dimensions mesurables.
- Une clarification du rôle de la stabilité comme dimension à part entière de la fiabilité, méritant un protocole d'évaluation dédié ([Ch. 6](#ch6)).
- Une lecture critique des [*frameworks*](#gloss-framework) d'évaluation existants (RAGAS, TruLens, [*LLM-as-judge*](#gloss-llm-as-judge)), avec mise en évidence de leurs limites en domaine critique.

#### Apports méthodologiques {#sec11-3-2}

- Un catalogue structuré des leviers techniques d'un RAG avec leurs compromis ([Ch. 4](#ch4)), réutilisable pour tout projet RAG d'entreprise.
- Un protocole d'évaluation diagnostique ([Ch. 5](#ch5)) organisé par dimension, qui permet de localiser l'origine des erreurs plutôt que de juger globalement.
- Un protocole de stabilité ([Ch. 6](#ch6)) directement applicable.

#### Apports industriels (cas ScribBERT) {#sec11-3-3}
...
- Une identification claire des limites du prototype (hybridation, [*reranking*](#gloss-reranking), gestion des tableaux et images) et un plan d'amélioration priorisé.

### Recommandations pour évaluer un RAG en contexte critique {#sec11-4}
...
3. Évaluer chaque composant avant l'évaluation [*end-to-end*](#gloss-end-to-end) pour permettre le diagnostic.
4. Tester systématiquement la stabilité (pas seulement la qualité moyenne).
5. Combiner [*LLM-as-judge*](#gloss-llm-as-judge) et validation humaine sur un échantillon pour calibrer.
6. Mesurer le coût opérationnel (latence, €) en parallèle de la qualité.
7. Versionner et *journaliser* tout, dès la phase de prototypage : ce qui n'est pas tracé ne peut être ni reproduit ni réutilisé.
8. Anticiper la conformité AI Act et les enjeux de responsabilité dès la conception, pas après le déploiement.

### Perspectives {#sec11-5}

#### Améliorations techniques court terme (ScribBERT) {#sec11-5-1}

- Hybridation BM25 + dense pour améliorer le rappel sur les références exactes.
- [*Reranker*](#gloss-reranker) [*cross-encoder*](#gloss-cross-encoder) pour la précision du top-$k$ injecté.
- Image-to-text contextualisé pour intégrer tableaux et schémas.
- Enrichissement des métadonnées des [*chunks*](#gloss-chunk).
- Refactoring du [*prompt*](#gloss-prompt) système : itérer sur la formulation des instructions pour traiter à la source plusieurs des biais identifiés au [§ 9.5](#sec9-5) (biais de longueur, biais d'ordre des citations) sans modifier la chaîne de traitement de récupération.
- Évaluation systématique selon le protocole [Ch. 5](#ch5)-[6](#ch6).

#### Pistes de recherche moyen terme {#sec11-5-2}

- [*Fine-tuning*](#gloss-fine-tuning) d'un modèle de vectorisation sur le corpus santé-sécurité (apprentissage contrastif sur paires question/passage), pour combler l'absence de modèle spécialisé santé-sécurité/BTP identifié au [Ch. 4.1.1](#sec4-1-1).
- [*GraphRAG*](#gloss-graphrag) : exploiter une représentation en graphe des entités santé-sécurité (procédures, EPI, risques, situations) pour des requêtes nécessitant un raisonnement multi-saut.
- [*Agentic*](#gloss-agentic) RAG : pour les questions complexes, décomposer en sous-questions, lancer plusieurs [*retrievals*](#gloss-retrieval), agréger.
- RAG multimodal : intégrer images, schémas, vidéos de formation comme sources de premier niveau.

#### Généralisation à d'autres domaines {#sec11-5-3}
...
#### Enjeux éthiques et de responsabilité à long terme {#sec11-5-4}
...
Les annexes ci-dessous documentent les matériaux mobilisés pour la mise en œuvre du protocole d'évaluation décrit en [Partie II](#partie-2) et instancié sur ScribBERT en [Partie III](#partie-3). Elles sont référencées dans le texte principal lorsque c'est utile et permettent au lecteur de vérifier ou de reproduire les choix décrits.

## Annexe A : Échantillon du jeu de test {-} {#annexe-A}

Le jeu de test interne utilisé pour le [*benchmark*](#gloss-benchmark) ([§ 8.1.2](#sec8-1-2)) comporte 50 questions stratifiées par type, difficulté, criticité et langue, conformément à la grille du [Ch. 5.3.2](#sec5-3-2). Le tableau ci-dessous reproduit un échantillon de 10 questions représentatives sur les 50 du jeu de test complet, choisies pour couvrir les six types d'intention identifiés ainsi que les trois niveaux de difficulté et de criticité.
...
- Q017 (*conditionnelle, difficile, fr*) : « Dans quels cas un plan de prévention sous-traitant est-il obligatoire ? », question dont la réponse n'est *pas* dans le corpus (test du refus contrôlé, cf. [§ 9.3](#sec9-3)).
...
- Q032 (*procédurale, moyenne, fr*) : « Quels documents et vérifications sont nécessaires avant d'autoriser un sous-traitant à intervenir pour la première fois sur un chantier BYTP ? », question multi-documents testant la complétude d'une liste de prérequis (documents entreprise + accueil sécurité individuel).
...
Les paraphrases associées à chaque question (1 à 3 reformulations préservant l'intention) sont utilisées pour le protocole de stabilité décrit au [Ch. 6](#ch6) et exécuté au [§ 8.4](#sec8-4).

## Annexe B : Configurations testées dans le *benchmark* {-} {#annexe-B}

Le plan factoriel exécuté en phase exploratoire ([§ 7.5.2](#sec7-5-2) et [§ 8.1](#sec8-1)) couvre 16 modèles de vectorisation × 9 stratégies de [*chunking*](#gloss-chunking) × 6 variantes de récupération = 864 cellules, dont 750 ont produit des résultats exploitables.

**Modèles de vectorisation (16 retenus sur 18 envisagés) :** `ada-002`, `embed-3-large` (OpenAI via Azure) ; `e5-small-ml`, `e5-base-ml`, `e5-large-ml`, `bge-m3`, `jina-v3`, `nomic-v2`, `granite-311m-ml`, `qwen3-embed-8b` (multilingues *open-source*) ; `minilm-l6`, `mpnet-base`, `jina-v2-base-en` (généralistes anglais) ; `camembert-large`, `solon-large`, `bilingual-fr-en` (francophones / bilingues). Deux modèles supplémentaires (`gte-qwen2-7b` et `nv-embed-v2`) avaient initialement été inclus dans le plan, mais ont dû être écartés en bloc faute d'avoir pu être initialisés dans l'environnement local (incompatibilités avec les versions installées de `transformers` / `sentence-transformers`, cf. § 7.5.2) : ils ne figurent dans aucun résultat reporté.
...
**Variantes de récupération (6) :** `dense-k5`, `dense-k10`, `dense-k5-thresh` (seuil de similarité), `dense-k5-neigh` (voisinage *n−1* / *n+1*), `hybrid-k5` (dense + BM25, fusion RRF), `dense-k20-rerank5` ([*reranking*](#gloss-reranking) [*cross-encoder*](#gloss-cross-encoder) `bge-reranker-v2-m3`).

**Configurations de génération (5) évaluées RAGAS** ([§ 8.3](#sec8-3)) : trois côté Azure (`azure-gpt35` × récupération {`markdown-1200-50` + `ada-002` + `dense-k5-thresh`, `markdown-1200-50` + `ada-002` + `dense-k5-neigh`, `recursive-512-64` + `ada-002` + `hybrid-k5`}) et deux côté local (`local-mistral7b` × récupération {`fixed-256-0` + `minilm-l6` + `dense-k5-neigh`, `recursive-512-64` + `e5-base-ml` + `dense-k5-neigh`}).


## Annexe C : Grille d'évaluation humaine instanciée {-} {#annexe-C}

La grille générique du [Ch. 5.2.2](#sec5-2-2) a été instanciée pour ScribBERT comme suit, sur six critères pondérés. Elle est destinée à être renseignée par des experts P2S sur l'échantillon de questions critiques identifié au [§ 11.2.1](#sec11-2-1).
...
- []{#gloss-advanced-rag}*Advanced RAG* : Famille d'architectures RAG « avancées » qui dépassent le schéma *retriever-reader* basique en empilant plusieurs étages (récupération large, filtrage, *reranking*, génération conditionnée), parfois enrichies de boucles de réflexion ou de décision sur la nécessité de récupérer.
- []{#gloss-agentic}*agentic* : Qualifie un système d'IA capable d'agir de façon autonome en enchaînant plusieurs actions (recherches, appels d'outils, raisonnements) pour atteindre un objectif.
- []{#gloss-answer-relevance}*answer relevance* (variantes : *answer relevancy*) : Pertinence de la réponse : mesure à quel point la réponse générée traite effectivement la question posée (métrique RAGAS).
- []{#gloss-approximate-nearest-neighbor}*Approximate Nearest Neighbor* : Recherche du plus proche voisin approximatif : famille d'algorithmes (HNSW, IVF, PQ…) qui accélèrent la recherche vectorielle au prix d'une légère approximation.
- []{#gloss-audit-trail}*audit trail* : Piste d'audit : trace complète et vérifiable des étapes ayant conduit à une réponse (passages récupérés, *prompt*, modèle, paramètres).
- []{#gloss-backend}*backend* : Partie serveur d'une application (logique métier, accès aux données), par opposition au *frontend*.
- []{#gloss-baseline}*baseline* : Configuration de référence à laquelle des variantes sont comparées pour mesurer un gain ou une perte.
- []{#gloss-batching}*batching* : Regroupement de plusieurs requêtes ou éléments en un seul lot pour améliorer le débit (souvent au prix de la latence individuelle).
- []{#gloss-benchmark}*benchmark* : Jeu de données et protocole standardisés permettant de comparer des systèmes ou des modèles sur une tâche donnée.
- []{#gloss-chatbot}*chatbot* : Agent conversationnel textuel.
- []{#gloss-chunk}*chunk* : Segment de texte issu du découpage d'un document, unité de base indexée et récupérée dans un système RAG.
- []{#gloss-chunker}*chunker* : Composant logiciel qui réalise le découpage en *chunks*.
- []{#gloss-chunking}*chunking* : Étape de découpage des documents en segments (*chunks*) avant indexation.
- []{#gloss-citation-completeness}*citation completeness* : Complétude des citations : toutes les affirmations qui devraient être sourcées le sont-elles ?
- []{#gloss-citation-correctness}*citation correctness* : Correction des citations : les passages cités existent-ils et soutiennent-ils réellement l'affirmation ?
- []{#gloss-citation-faithfulness}*citation faithfulness* : Fidélité de la citation : le passage cité supporte-t-il bien l'affirmation à laquelle il est rattaché ?
- []{#gloss-cluster}*cluster* : Groupe d'éléments homogènes obtenus par regroupement automatique (*clustering*).
- []{#gloss-clustering}*clustering* : Regroupement automatique d'éléments en *clusters* sur la base d'une similarité (vectorielle, lexicale, structurelle).
- []{#gloss-code-switching}*code-switching* : Alternance codique : passage spontané d'une langue à l'autre au sein d'un même énoncé (ici, français ↔ anglais dans les requêtes).
- []{#gloss-context-precision}*context precision* : Précision du contexte : proportion des passages récupérés qui sont effectivement pertinents (métrique RAGAS).
- []{#gloss-context-recall}*context recall* : Rappel du contexte : proportion de l'information de référence couverte par les passages récupérés (métrique RAGAS).
- []{#gloss-context-relevance}*context relevance* : Pertinence du contexte : mesure de l'utilité globale des passages récupérés pour répondre à la question.
- []{#gloss-cross-encoder}*cross-encoder* : Encodeur croisé : modèle qui prend simultanément la requête et le passage en entrée pour produire un score de pertinence fin (utilisé en *reranking*).
- []{#gloss-custom}*custom* : Personnalisé, sur mesure (par opposition à une solution générique « prête à l'emploi »).
- []{#gloss-dense-retrieval}*dense retrieval* : Recherche dense : récupération de passages via similarité entre vectorisations denses de la requête et des documents.
- []{#gloss-dual-encoder}*dual-encoder* : Architecture à deux encodeurs (souvent identiques) qui encodent séparément requête et passage avant comparaison ; synonyme de *bi-encodeur*.
- []{#gloss-embedding}*embedding* : Représentation vectorielle dense d'un mot, d'une phrase ou d'un document dans un espace continu.
- []{#gloss-end-to-end}*end-to-end* : Bout en bout : qui couvre l'intégralité de la chaîne, de l'entrée brute jusqu'au résultat final.
- []{#gloss-endpoint}*endpoint* : Point d'accès réseau (URL) exposant une API.
- []{#gloss-exact-match}*exact match* : Correspondance exacte : la réponse générée doit être strictement identique à la référence.
- []{#gloss-faithfulness}*faithfulness* : Fidélité : propriété d'une réponse dont toutes les propositions sont effectivement supportées par les passages fournis.
- []{#gloss-few-shot}*few-shot* : Apprentissage à partir de quelques exemples seulement fournis dans le *prompt*.
- []{#gloss-fine-tuning}*fine-tuning* : Affinage : adaptation d'un modèle pré-entraîné à une tâche ou un domaine spécifique via un entraînement supplémentaire.
- []{#gloss-flip-rate}*flip rate* : Taux de bascule : proportion de cas où, d'une exécution à l'autre, le verdict (bon/mauvais, supporté/non supporté…) change.
- []{#gloss-framework}*framework* : Cadriciel : ensemble cohérent d'outils et de conventions facilitant le développement (ex. LangChain, LlamaIndex).
- []{#gloss-frontend}*frontend* : Partie cliente d'une application (interface utilisateur), par opposition au *backend*.
- []{#gloss-gap-analysis}*gap analysis* : Analyse d'écarts : fonctionnalité permettant de lancer la même requête sur plusieurs sous-ensembles de documents sélectionnés et de comparer les réponses obtenues pour visualiser les différences entre référentiels.
- []{#gloss-gold-standard}*gold standard* : Référence absolue : annotation considérée comme la vérité de terrain pour évaluer un système.
- []{#gloss-graphrag}*GraphRAG* : Variante de RAG s'appuyant sur un graphe de connaissances pour structurer la récupération et l'agrégation d'information.
- []{#gloss-grounding}*grounding* : Ancrage explicite d'une génération sur des sources externes vérifiables.
- []{#gloss-groundedness}*groundedness* : Synonyme anglo-saxon de la fidélité aux sources : propriété d'une réponse dont les propositions sont justifiées par les passages effectivement fournis (proche de *faithfulness*).
- []{#gloss-hard-negatives}*hard negatives* : Exemples négatifs « difficiles » utilisés à l'entraînement d'un modèle de vectorisation : passages thématiquement proches d'un positif mais non pertinents, qui forcent le modèle à mieux discriminer.
- []{#gloss-human-in-the-loop}*human-in-the-loop* : Humain dans la boucle : protocole où un opérateur humain valide ou corrige les sorties du système.
- []{#gloss-inline}*inline* : En ligne : intégré directement dans le flux (ex. citation insérée dans le texte de la réponse).
- []{#gloss-late-interaction}*late interaction* : Interaction tardive : famille d'architectures (ex. ColBERT) qui combinent l'efficacité d'un *bi-encodeur* avec des interactions fines au niveau des *tokens*.
- []{#gloss-leaderboards}*leaderboards* : Tableaux de classement publics comparant les performances de modèles sur un *benchmark* donné (ex. MTEB).
- []{#gloss-learning-to-rank}*learning-to-rank* : Apprentissage d'ordonnancement : famille de méthodes apprenant à classer des documents par pertinence à partir de données annotées.
- []{#gloss-listwise}*listwise* : Approche d'apprentissage d'ordonnancement (*learning-to-rank*) qui optimise directement le classement d'une liste entière de documents, par opposition aux approches *pointwise* (un score par document) et *pairwise* (comparaison par paires).
- []{#gloss-llm-as-judge}*LLM-as-judge* (variante francisée : *LLM-juge*) : LLM utilisé comme évaluateur automatique pour noter d'autres réponses selon une grille.
- []{#gloss-loader}*loader* : Chargeur : composant qui lit des données depuis une source et les rend exploitables.
- []{#gloss-logger}*logger* : Composant logiciel qui enregistre des événements ou des métriques d'exécution.
- []{#gloss-lost-in-the-middle}*lost in the middle* : Phénomène par lequel un LLM exploite moins bien les passages situés au milieu d'un long contexte qu'en début ou en fin.
- []{#gloss-machine-verifiable}*machine-vérifiable* : Vérifiable automatiquement par une machine, sans intervention humaine.
- []{#gloss-mapping}*mapping* : Correspondance : table reliant des éléments d'un ensemble à ceux d'un autre.
- []{#gloss-massive-text-embedding-benchmark}*Massive Text Embedding Benchmark* : MTEB : *benchmark* de référence couvrant de nombreuses tâches d'évaluation des modèles de vectorisation.
- []{#gloss-matryoshka-representation-learning}*Matryoshka Representation Learning* : Famille de vectorisations dont les premières dimensions portent déjà l'essentiel de l'information, permettant une troncature *a posteriori*.
- []{#gloss-max-tokens}*max tokens* : Borne supérieure du nombre de *tokens* qu'un LLM peut générer en sortie pour une requête donnée.
- []{#gloss-mean-reciprocal-rank}*Mean Reciprocal Rank* : Rang réciproque moyen (MRR) : moyenne des inverses du rang du premier document pertinent.
- []{#gloss-memex}*Memex* : Concept de bureau documentaire mécanisé proposé par Vannevar Bush en 1945, souvent considéré comme l'ancêtre conceptuel de la recherche d'information moderne.
- []{#gloss-multi-query}*multi-query* : Stratégie consistant à reformuler la requête en plusieurs variantes pour augmenter le rappel de la récupération.
- []{#gloss-multi-stage}*multi-stage* : Architecture de récupération en plusieurs étages successifs (récupération large, *reranking*, sélection finale), qui combine efficacité et précision.
- []{#gloss-one-factor-at-a-time}*One-Factor-At-a-Time* : OFAT : protocole expérimental consistant à ne faire varier qu'un seul paramètre à la fois, toutes choses égales par ailleurs.
- []{#gloss-open-source}*open-source* (variante : *open-weights* quand seuls les poids sont diffusés, sans le code d'entraînement) : À code source (et/ou poids) ouvert.
- []{#gloss-output}*output* : Sortie d'un système.
- []{#gloss-overlap}*overlap* : Recouvrement : portion de texte commune entre deux *chunks* consécutifs, qui amortit les coupures.
- []{#gloss-pairwise}*pairwise* : Approche d'apprentissage d'ordonnancement (*learning-to-rank*) qui apprend à comparer des paires de documents et à préférer le plus pertinent.
- []{#gloss-parent-document-retrieval}*parent-document retrieval* : Récupération du document parent : de petits *chunks* sont indexés mais le passage parent plus large est retourné au LLM.
- []{#gloss-parser}*parser* : Analyseur syntaxique : composant qui transforme une entrée brute en structure exploitable.
- []{#gloss-pipeline}*pipeline* : Chaîne de traitement composée d'étapes successives.
- []{#gloss-pointwise}*pointwise* : Approche d'apprentissage d'ordonnancement (*learning-to-rank*) qui prédit indépendamment un score de pertinence par document, sans tenir compte des autres candidats.
- []{#gloss-prompt}*prompt* : Instruction ou message fourni en entrée à un LLM pour orienter sa génération.
- []{#gloss-query-expansion}*query expansion* : Expansion de requête : enrichissement automatique de la requête par des termes liés (synonymes, paraphrases).
- []{#gloss-query-likelihood}*query likelihood* : Modèle probabiliste estimant la vraisemblance que la requête ait été générée par un document.
- []{#gloss-query-rewriting}*query rewriting* : Réécriture de requête par un modèle (correction, normalisation, reformulation).
- []{#gloss-reciprocal-rank-fusion}*Reciprocal Rank Fusion* : RRF : méthode robuste de fusion de plusieurs classements via la somme des inverses des rangs.
- []{#gloss-recursive-splitter}*recursive splitter* : Découpeur récursif (notamment *recursive character text splitter* de LangChain) qui tente d'abord les séparateurs « forts » (paragraphes, phrases) avant de tomber sur du découpage caractère par caractère.
- []{#gloss-relevance-feedback}*relevance feedback* : Retour de pertinence : technique IR consistant à reformuler ou enrichir une requête à partir des documents jugés pertinents lors d'une première itération.
- []{#gloss-reranker}*reranker* : Composant qui effectue le *reranking* (souvent un *cross-encoder*).
- []{#gloss-reranking}*reranking* : Reclassement d'un petit ensemble de candidats par un modèle plus précis (et plus coûteux) que le *retriever* initial.
- []{#gloss-retrieval}*retrieval* : Récupération : phase consistant à retrouver, dans un index, les passages pertinents pour une requête.
- []{#gloss-retriever}*retriever* : Composant chargé de la récupération.
- []{#gloss-retriever-reader}*retriever-reader* : Architecture historique des systèmes de questions-réponses en deux étages : un module de récupération sélectionne des passages, puis un module de lecture en extrait la réponse.
- []{#gloss-screening}*screening* : Tri préliminaire : présélection rapide de candidats avant analyse plus poussée.
- []{#gloss-seed}*seed* : Valeur d'initialisation d'un générateur pseudo-aléatoire : son fixage permet la reproductibilité des résultats d'une exécution à l'autre.
- []{#gloss-siamese-networks}*siamese networks* : Réseaux siamois : architecture à deux branches partageant les mêmes poids, utilisée pour apprendre des similarités.
- []{#gloss-sparse-retrieval}*sparse retrieval* : Recherche creuse : récupération fondée sur des représentations à très haute dimension et majoritairement nulles (BM25, TF-IDF).
- []{#gloss-splitter}*splitter* : Découpeur : composant qui segmente un texte en *chunks*.
- []{#gloss-stack}*stack* (ou pile technologique) : ensemble des outils, bibliothèques et services utilisés dans un projet.
- []{#gloss-step-back-prompting}*step-back prompting* : Stratégie consistant à reformuler la question en une question plus générale avant la recherche, pour mieux ancrer la réponse.
- []{#gloss-tenant}*tenant* : Locataire : isolement logique d'un client dans une infrastructure mutualisée (ex. *tenant* Azure).
- []{#gloss-term-specificity}*term specificity* : Spécificité d'un terme : mesure introduite par Sparck Jones (1972) du caractère discriminant d'un mot dans une collection ; fondement théorique de l'IDF.
- []{#gloss-time-consuming}*time-consuming* : Chronophage.
- []{#gloss-token}*token* : Unité élémentaire de texte manipulée par un LLM (mot, sous-mot ou caractère selon le *tokenizer*).
- []{#gloss-tokenizer}*tokenizer* : Tokeniseur : composant qui découpe un texte brut en *tokens* selon un vocabulaire et un algorithme donnés (BPE, WordPiece, SentencePiece, etc.).
- []{#gloss-top-k}*top-k* : Les $k$ premiers résultats d'un classement (ex. top-5 passages récupérés).
- []{#gloss-top-p}*top-p* : Échantillonnage *top-p* (ou *nucleus sampling*) : limite la génération aux *tokens* dont la masse de probabilité cumulée atteint $p$.
- []{#gloss-vision-language-model}*Vision Language Model* : VLM : modèle multimodal qui traite conjointement images et texte.
- []{#gloss-watermark}*watermark* : Filigrane : signal discret inséré dans une sortie pour en tracer l'origine.
- []{#gloss-workflow}*workflow* : Flux de travail : séquence d'étapes coordonnées composant un processus.
